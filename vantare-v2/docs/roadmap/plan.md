...
- cuerpo: Eficiencia ofrece los estilos Signature y Broadcast para Standings, con transparencia suave, identidad Vantare, tipografía unificada y selección neutra. Cabecera y pie fino permiten elegir información de sesión; las diagonales reflejan la bandera sin animación cuando existe una señal vigente. La fuente REST LMU aporta temperaturas con calidad y caducidad y una bandera fail-closed; su equivalencia de códigos exige comprobación en sesión activa. Los perfiles conservan la selección y el marco adapta columnas, filas y pie en Studio, Desktop y OBS. La entrega conjunta quedó validada físicamente e integrada en Nightly mediante PR #1107; la equivalencia de bandera y temperatura sigue pendiente de una sesión LMU activa. La recuperación de los ajustes locales de Delta, Relative, Standings, Pedals y Fuel Strategy ha pasado las pruebas funcionales y los controles de calidad; la revisión visual del candidato sigue pendiente antes de integrarse. En el candidato de revisión, el número de Delta mantiene su escala al cruzar por cero, sin el pulso de aumento; la barra y los cambios de color se conservan. Delta cuenta con aprobación visual de Isaac. En el candidato de Workshop, Standings conserva la clasificación elegida al reproducir escenas por piloto, usa datos de ejemplo coherentes por sesión y permite comprobar el estado PIT con un rival visible.
- cuerpo.en: Efficiency offers Signature and Broadcast Standings styles, with subtle transparency, Vantare identity, unified typography and neutral selection. The header and thin footer offer selectable session information; diagonal accents reflect the flag without animation when a current signal exists. The LMU REST source supplies temperatures with quality and expiry plus a fail-closed flag; its code mapping requires an active-session check. Profiles retain selections and the frame fits columns, rows and footer across Studio, Desktop and OBS. The joint delivery was physically validated and integrated into Nightly through PR #1107; flag and temperature equivalence still awaits an active LMU session. Recovery of local Delta, Relative, Standings, Pedals and Fuel Strategy refinements has passed functional tests and quality checks; visual review of the candidate is still required before integration. In the review candidate, the Delta number keeps its scale when crossing zero, without the enlargement pulse; the bar and colour changes are preserved. Delta has Isaac’s visual approval. In the Workshop candidate, Standings preserves the selected classification while playing per-driver scenes, uses coherent sample data for each session and lets users check PIT status with a visible rival.
- cuerpo.pt: Eficiência oferece os estilos Signature e Broadcast para Standings, com transparência suave, identidade Vantare, tipografia unificada e seleção neutra. Cabeçalho e rodapé fino permitem escolher informação da sessão; as diagonais refletem a bandeira sem animação quando existe um sinal atual. A fonte REST LMU fornece temperaturas com qualidade e expiração e uma bandeira fail-closed; o mapeamento dos códigos exige verificação numa sessão ativa. Os perfis preservam a seleção e o quadro adapta colunas, filas e rodapé no Studio, Desktop e OBS. A entrega conjunta foi validada fisicamente e integrada na Nightly através do PR #1107; a equivalência da bandeira e da temperatura continua pendente de uma sessão LMU ativa. A recuperação dos ajustes locais de Delta, Relative, Standings, Pedals e Fuel Strategy passou nos testes funcionais e controlos de qualidade; a revisão visual do candidato continua pendente antes da integração. No candidato em revisão, o número do Delta mantém a escala ao cruzar zero, sem o pulso de aumento; a barra e as mudanças de cor são preservadas. Delta tem aprovação visual de Isaac. No candidato do Workshop, Standings preserva a classificação escolhida ao reproduzir cenas por piloto, usa dados de exemplo coerentes por sessão e permite verificar o estado PIT com um adversário visível.
- cuerpo.it: Efficienza offre gli stili Signature e Broadcast per Standings, con trasparenza lieve, identità Vantare, tipografia unificata e selezione neutra. Intestazione e piè sottile permettono di scegliere informazioni di sessione; le diagonali riflettono la bandiera senza animazione quando esiste un segnale attuale. La fonte REST LMU fornisce temperature con qualità e scadenza e una bandiera fail-closed; la mappatura dei codici richiede una verifica in sessione attiva. I profili conservano la selezione e il riquadro adatta colonne, righe e piè in Studio, Desktop e OBS. La consegna congiunta è stata verificata fisicamente e integrata in Nightly tramite la PR #1107; l'equivalenza di bandiera e temperatura resta in attesa di una sessione LMU attiva. Il recupero degli aggiornamenti locali di Delta, Relative, Standings, Pedals e Fuel Strategy ha superato i test funzionali e i controlli di qualità; la verifica visiva del candidato resta necessaria prima dell’integrazione. Nel candidato in revisione, il numero Delta mantiene la scala attraversando lo zero, senza l’impulso di ingrandimento; la barra e i cambi di colore restano invariati. Delta ha l’approvazione visiva di Isaac. Nel candidato Workshop, Standings conserva la classificazione scelta durante le scene per pilota, usa dati di esempio coerenti per sessione e permette di verificare lo stato PIT con un avversario visibile.
- etiqueta: ISA-1083
