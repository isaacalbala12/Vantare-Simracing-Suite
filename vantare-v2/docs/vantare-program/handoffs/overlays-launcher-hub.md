...
El seguimiento está en [PR draft #1354](https://github.com/isaacalbala12/Vantare-Simracing-Suite/pull/1354), rama `vantareapp/isa-1347-nightly-parser-perf` desde el SHA integrado. El primer intento pasó promotion path y ratchet, pero el gate frontend volvió a medir compacto en 1,656 ms / 1,5 ms (`1be6fdf9`); no se rebajó el límite. El commit de código `bae7c02e` reutiliza el buffer UTF-8 y exige que `encodeInto` lea el texto completo, manteniendo límite exacto para Unicode; la regresión prueba el borde de 72 KiB. Localmente: suite completa del primer intento, 481 archivos y 4.036 PASS (2 omitidas); suite focal final 24/24, legacy 0,684 ms y compacto 0,844 ms; typecheck, lint y build PASS. CI del head actualizado pendiente.
