...
Seguimiento correctivo en la rama `vantareapp/isa-1347-nightly-parser-perf`, basada en ese SHA de Nightly. El primer head `17c0e91a` optimizó el tamaño UTF-8 con un patrón ASCII; el gate frontend volvió a fallar en compacto a 1,656 ms frente a 1,5 ms (head `1be6fdf9`). El commit `bae7c02e` sustituyó el escaneo por `TextEncoder.encodeInto` sobre un buffer UTF-8 reutilizado y preserva los 72 KiB exactos, pero el gate midió 1,624 ms. El commit `264ceb86` añade además la normalización directa del formato compacto común de Standings, sin bucles de alias; conserva validación estricta y las regresiones de cero, precisión y calidad missing. PR draft [#1354](https://github.com/isaacalbala12/Vantare-Simracing-Suite/pull/1354), CI del head actualizado pendiente.

Evidencia local: pruebas dirigidas de contrato/rendimiento/compact-wire sobre el último código 41/41; muestra legacy 0,748 ms y compacto 0,814 ms; suite completa sobre el primer intento 481 archivos, 4.036 PASS y 2 omitidas; typecheck, lint y build pasan sobre el código nuevo. El gate remoto de #1354 debe validar el head exacto. Sin cambios de roadmap, promoción, release ni afirmación de prueba física; LMU activo en Windows/Desktop/OBS sigue pendiente.
