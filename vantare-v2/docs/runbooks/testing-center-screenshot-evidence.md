...
4. Llamar `testing_center_finalize_screenshot`. La RPC comprueba ownership,
   objeto exacto y estado, y reserva una sola validación durable.
...
- No hay policies cliente de `SELECT`, `UPDATE` o `DELETE`; tampoco se concede
  `upsert`.
- Identidad, rol, canal autorizado, IDs y paths se derivan server-side.
...
El primer runner crea PostgreSQL desechable, aplica el historial, exige 62/62
aserciones pgTAP, ejecuta rollback exacto, reaplica y repite 62/62, prueba una
carrera real de dos procesos sobre `finalize` y verifica que un bucket previo
incompatible falla cerrado. El segundo protege la compatibilidad de la RPC v1.
...
coincide, elimina sus objetos, RPCs, policy, tablas y bucket de ISA-350, y
restaura exactamente el conjunto anterior de tipos de evidencia.
