...
- cuerpo: Go decide uno de cinco niveles efectivos, publica `capabilities.performance` y aplica una cadencia común; Ajustes permite elegirlos y cada perfil v4 puede heredar, fijar un nivel o personalizar Hz y efectos por widget con su coste visible. Studio guarda esa política y deja `updateHz` como dato exclusivo del lector legado. D8 está demostrado para spotter por ruta real; banderas no tiene señal canónica en v2 y se verifica en #893. El valor inicial conserva la paridad hasta superar el banco de rendimiento.
- cuerpo.en: Go selects one of five effective levels, publishes `capabilities.performance` and applies one shared cadence; Settings exposes those levels and each v4 profile can inherit, select a level, or customise per-widget Hz and effects with visible cost. Studio saves that policy and keeps `updateHz` exclusively in the legacy reader. D8 is proven for spotter through the real path; flags have no canonical v2 signal and are verified in #893. The initial value preserves parity until the performance benchmark passes.
- cuerpo.pt: O Go decide um de cinco níveis efetivos, publica `capabilities.performance` e aplica uma cadência comum; as Definições permitem escolhê-los e cada perfil v4 pode herdar, fixar um nível ou personalizar Hz e efeitos por widget com o custo visível. O Studio guarda essa política e mantém `updateHz` apenas no leitor legado. D8 está demonstrado para o spotter pela rota real; as bandeiras não têm sinal canónico em v2 e são verificadas em #893. O valor inicial preserva a paridade até o banco de desempenho ser aprovado.
- cuerpo.it: Go decide uno dei cinque livelli effettivi, pubblica `capabilities.performance` e applica una cadenza comune; le Impostazioni permettono di sceglierli e ogni profilo v4 può ereditare, fissare un livello o personalizzare Hz ed effetti per widget mostrando il costo. Studio salva questa politica e mantiene `updateHz` solo nel lettore legacy. D8 è dimostrato per lo spotter tramite il percorso reale; le bandiere non hanno un segnale canonico v2 e sono verificate in #893. Il valore iniziale mantiene la parità fino al superamento del benchmark prestazionale.
- etiqueta: Feature
