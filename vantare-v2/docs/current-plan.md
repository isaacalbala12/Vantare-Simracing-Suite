...
- Evidencia fresca de segunda review: comprobación RF/SC PASS sobre 143 filas de campos/grupos; test focal y `go test ./internal/telemetry/... -count=1` PASS; `git diff --check` PASS. Dos repeticiones literales de `go test ./... -count=1` quedaron rojas por locks/timing Windows fuera de este diff (`TestConcurrentSavesDontCorruptFile` y, en una repetición, `TestServiceEmitRateCapped`); ambos tests pasan aislados y no se modificaron. Una ejecución serial superó `internal/app` y fue terminada externamente antes de acabar el resto.
