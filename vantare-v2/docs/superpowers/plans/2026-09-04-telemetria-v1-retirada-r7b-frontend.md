...
- Rollback/stop: revert. Stop si queda un caller productivo → volver a C2, no
  reabrir la prop.
- Resultado D1 (APROBADO final 2026-09-05): `e92d58dc` (RED 22/1, fallo
  estructural exigido) + `556c68ed`
  ...
  typecheck verde. `plan.md`/`roadmap.json` sin tocar (deuda del PR R7b, F2).
...
- Resultado D2 (APROBADO final en rama 2026-09-05):
  ...
  renderer y builders del oráculo; los dos oráculos los invocan directamente,
  sin registro paralelo ni fallback silencioso. Fuel `makeFrame` queda
...
- Resultado D3 (APROBADO final en rama 2026-09-05):
  ...
  P0/P1/P2 = 0; P3 = 1 informativo por casts temporales propiedad E4.
...
- Resultado D4 (APROBADO final en rama 2026-09-05):
  ...
  `missing` de daño por rutas directas. RED 21/23; focales 64/64, 140/140 y
...
Suite completa 407/407 ficheros y 3164/3164 tests PASS; Go completo y contrato
...
actualizó evidencia, handoff y roadmap, con digest `--check` verde. Las suites
completas pasaron secuencialmente sobre el cierre; los timeouts observados al
ejecutarlas en paralelo pasaron aislados y no son regresiones. F1 termina con
dos reviews independientes sobre el árbol final; no hay push, PR, merge ni
promoción.
