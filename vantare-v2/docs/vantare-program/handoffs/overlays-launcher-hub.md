...
### VAN-734 — comparación WebSocket/Streams en macOS (2026-09-22)

En la rama aislada `spike/wails-beta-streams-mac`, el laboratorio `tools/wails-transport-benchmark` comparó ráfagas con acuse de 120 tramas en una WebView Wails beta. Medianas de tres ejecuciones: 8 KiB WebSocket 39,5 ms/11,2 ms CPU Go, Streams 54,2 ms/70,1 ms; 64 KiB WebSocket 47,3 ms/16,0 ms, Streams 67,7 ms/83,0 ms. Todas las tramas llegaron ordenadas. El intento de cadencia 60 Hz se descartó por ralentización de temporizadores WebKit. Evidencia y límites: `docs/analysis/2026-09-22-wails-beta-streams-macos-spike.md`. No se ha cambiado el transporte productivo. Siguiente decisión: conservar el socket y, cuando haya Windows, probar una implementación integrada con `OverlayPullTransport`, LMU y WebView2 antes de reconsiderar Streams.
