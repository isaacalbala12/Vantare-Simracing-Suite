...
- Base rebasada: `origin/nightly@ca166b38`.
...
- Una corrida de 60 s por nivel solo compara el corte: no es el baseline
...
Pendiente de ejecutar sin `-Forzar` cuando no exista `vantare-baseline*`.
Las corridas contaminadas realizadas antes de la coordinación de máquina se
descartaron y no se usan en esta tabla.
...
- `hub:can-suspend` espera hasta 500 ms. `false` o timeout conserva el Hub y
  registra la causa; el state machine cubre niveles 1–5 y recreación.
...
- Bounding box: un clúster usa su unión más 16 px; widgets en esquinas opuestas
  ...
  Wails; edición fuerza el monitor completo.
...
- Pendientes por coordinación de máquina: valores RAM limpios y reapertura
  Wails real. También quedan captura de paridad visual,
