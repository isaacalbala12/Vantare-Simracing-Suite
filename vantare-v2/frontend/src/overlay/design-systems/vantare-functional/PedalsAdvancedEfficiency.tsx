 ...
 */
export function PedalsAdvancedEfficiency({ model, effects }: WidgetRendererProps<PedalsTelemetryViewModel>) {
  ...
  return (
    <section className="vf-pedals-adv" data-widget-system="vantare-functional" data-widget-renderer="pedals-telemetry" data-status={model.status} data-effects={effects}>
      {model.status !== "ready" && <p className="vf-pedals-adv-status" role="status">{model.statusMessage ?? model.status}</p>}
      ...
      <div className="vf-pedals-adv-bars" role="group" aria-label="Pedal inputs">
      ...
      <svg className="vf-pedals-adv-wheel" viewBox="0 0 64 64" aria-hidden="true">
