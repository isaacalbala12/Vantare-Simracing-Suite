...
- Evidencia: focal LMU repetido 20 veces PASS; `go test ./internal/telemetry/... -count=1` y guard ADR PASS; fuzz REST 10 s PASS (~307.666 ejecuciones); benchmark 4,66–5,33 µs/op, 2.096 B/op y 23 allocs/op. Build frontend PASS después de instalar exactamente el lockfile en el worktree nuevo. Vet focal conserva únicamente los dos warnings `unsafe.Pointer` heredados de ISA-32; `-race` no arranca con `CGO_ENABLED=0` y `gcc` ausente.
- Suite global: con `frontend/dist` generado, todos los paquetes salvo `internal/app` pasan; reaparece ISA-118 (`TestConcurrentSavesDontCorruptFile`) por colisiones Windows de `app-settings.json.tmp`. La misma prueba focal `-count=20` falla también en la base exacta ISA-32 `539da686`, fuera del diff; no se corrige aquí.
