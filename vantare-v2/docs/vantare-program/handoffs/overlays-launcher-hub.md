...
## ISA-1332 — prueba optativa de carrusel (2026-09-23)

Tras integrar #1335 en nightly8b25d076, Isaac aclara el comportamiento deseado: los pilotos deben recorrer una cinta continua de derecha a izquierda, ocultándose detrás del bloque fijo de vuelta. Aprueba probarlo solo en la franja de pilotos. Seguimiento en Asana1218756818225223, En curso, y puente técnico #1332 existente.

El estudio `broadcast-tower-carousel` activa `driverCarousel` en el renderer productivo Eficiencia. Mantiene el modo discreto en las demás escenas, vuelta127/180 y datos fijos en esta prueba. Recorre la selección actual de filas; no amplía el campo ni cambia Crystal, modelos, contratos de telemetría o perfiles guardados. La aceptación visual y cualquier integración posterior quedan pendientes.

Implementación GPT-6 Astra 0b4eb7ae, compuesta por root con escena en fea57b8a. Root revisa diff, aislamiento del hook y cierre geométrico del rail. Pruebas: 16 renderer, 79 focales combinadas y suite completa475archivos/3956PASS con2omitidas; build/TypeScript, lint, ratchet aggregate PASS con NEW0/MOVED0 y policy_changed=false. Contrato roadmap valida únicamente milestones:functional-widget-design. Sin cambios Go ni dependencias. Preview servidor5177 actualizado y limpio, apertura de escena solicitada en Codex; inspección visual de Isaac pendiente. La interrupción de la primera ejecución de checks obligó a repetirla; el control de calidad se ejecutó tras completar build para disponer del dist embebido. No se declara prueba física Windows/OBS ni integración en nightly.
