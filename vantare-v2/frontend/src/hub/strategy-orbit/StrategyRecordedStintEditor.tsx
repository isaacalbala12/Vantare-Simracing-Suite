...
export function StrategyRecordedStintEditor({ plan, drivers, locked, onDirtyChange, onRecalculate, t }: {
  ...
  return <section className="strategy-recorded-stint-editor" aria-labelledby="stint-editor-title">
    <header><div><p>{t("strategy.stint.eyebrow")}</p><h3 id="stint-editor-title">{t("strategy.stint.title")}</h3></div><span>{t("strategy.stint.total")}: {plan.totalLaps}</span></header>
    <ol>
    ...
    <footer>
      {dirty ? <p role="status">{t("strategy.stint.stale")}</p> : <p>{t("strategy.stint.hint")}</p>}
