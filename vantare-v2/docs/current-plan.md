...
- Frontera aprobada: `schema` y sus dominios no importan `catalog`; `catalog` importa únicamente schema/standard library y no participa dinámicamente en el hot path. ISA-26 está `Done`; ISA-27 está `In Progress`; ISA-28 no está iniciada.
...
- Estado de entrega: ISA-27 preparada para `In Review` en PR draft apilado hacia ISA-26; sin merge. ISA-28 continúa sin iniciar y las semánticas LMU/presencia/calidad/tiempo/identidad quedan aplazadas.
