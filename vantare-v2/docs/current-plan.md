...
Nota ISA-707 / Deuda #677 Tanda 3 (2026-08-21, implementada en rama, sin promoción):
- `DefaultSectionCadence` pasa a Fast 50ms / Mid 100ms / Slow 250ms / DirtyCeiling 1s y `SectionRelative` se mueve de TierMid a TierSlow para que su dirty fino (ISA-695) regule; `standings` ya era slow. Benchmark @104 60 Hz: plana 449k ns/op 480 builds/s vs regulada 228k ns/op 57 builds/s, B/s idéntico, -46% B/op y -29 allocs; señales <5 µs/tick. Centinelas `TestCachedProjectorMatchesProjectV2ByteForByte` y `TestStandingsRelativeStayFresh` siguen verdes vía memoización.
- `derive.DefaultFuelUsageWindowProduct = 3` cableado explícito en `telemetry_core_runtime.go` y `telemetry_shadow.go` (ambos `derive.NewPipeline(Config{FuelUsageWindow: ...})`); `Pipeline.FuelUsageWindow()` expuesto y `TestFuelUsageWindowIsWiredExplicitly` falla si alguno vuelve a `Config{}`.
- Archivos: `cadence.go`, `cadence_*.go`, `derive/pipeline.go`, `telemetry_core_runtime.go`, `telemetry_shadow.go`, `fuel_window_wiring_test.go`; evidencia `docs/telemetry-core/evidence/isa-677-cadencias-activadas.md` + fragmento `ISA-707.json`.
- Rama `vantareapp/isa-707-cadencias-y-fuel-window` sobre `origin/nightly@217ba746`; sin push previo, sin integración, promoción ni release. Gates: `go build` solo `frontend/embed.go` por `dist`, `go vet` tres `unsafe.Pointer` heredados, `go test ./internal/telemetry/... ./internal/app/... -count=1` verde, `git diff --check` limpio.
