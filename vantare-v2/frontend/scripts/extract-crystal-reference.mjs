...
const canonicalEntries = [
  ...
  { id: 'pedals-telemetry', widgetType: 'pedals-telemetry', designId: 'pedals-telemetry-crystal', referenceSelector: '.hud-capsule-v1', width: 323, height: 86, htmlSection: 4 },
  { id: 'pedals-telemetry-compact', widgetType: 'pedals-telemetry-compact', designId: 'pedals-telemetry-compact-crystal', referenceSelector: '.cockpit-v2-low', width: 248, height: 92, htmlSection: 4 },
  { id: 'pedals', widgetType: 'pedals', designId: 'pedals-crystal', referenceSelector: '.cockpit-v3-solo', width: 120, height: 167, htmlSection: 4 },
...
function validateManifest(manifest) {
  if (manifest.version !== 2) fail(`unsupported manifest version: ${manifest.version}`);
  ...
  if (manifest.captureProtocol?.margin !== 128 || manifest.captureProtocol?.guard !== 8 || manifest.captureProtocol?.dpr !== 1) {
    fail('unexpected capture protocol geometry');
  }
  if (manifest.captureProtocol?.scenes?.map(({ id }) => id).join(',') !== 'transparent,solid,grid') {
    fail('unexpected capture protocol scenes');
  }
  ...
  for (const [index, expected] of canonicalEntries.entries()) {
    ...
    if (actual.capture?.protocol !== 2) fail(`entry ${actual.id} does not use capture protocol v2`);
    if (!(actual.capture?.scenes?.transparent?.alphaLt255Ratio > 0)) fail(`entry ${actual.id} lost transparency`);
    ids.add(actual.id);
