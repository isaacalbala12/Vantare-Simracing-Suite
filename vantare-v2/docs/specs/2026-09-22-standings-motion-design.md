...
Isaac pide incluir ambas animaciones entre las que puede ver en el harness. Además de sus escenas individuales, la secuencia combinada que tiene abierta reproduce batalla y ventana después de vueltas, posiciones y boxes. Conserva las posiciones ya alcanzadas y los tiempos de vuelta. Cuando el fotograma declara una ventana de revisión, Workshop aplica ese recorte también en V1, sin cambiar el estilo ni la clasificación elegidos. La reproducción toma muestras a 15/30 Hz y el avance manual llega al fotograma exacto; se protege el recorrido completo en Normal/Multiclase. Ajuste de demostración dentro del alcance aprobado, sin cambiar el motor ni el renderer productivo.

Tras la revisión de Isaac de los pasos 8–12, la demostración de ventana usa P1→P7→P9→P1, alcanzables con las diez filas por defecto, y conserva las distancias al pasar de batalla a boxes. El contador y la leyenda permanecen en el paso actual hasta que cambia el hecho discreto. Durante estas escenas se reserva la altura del widget sin recortar únicamente en el marco externo del harness: el renderer mantiene su geometría real y el preview no se recentra al entrar/salir filas. Un tamaño de preview explícito conserva su autoridad. Verificación visual de Isaac pendiente; los controles automáticos no la sustituyen.
