...
- Evidencia fresca de la corrección de review: focal LMU repetido 20 veces PASS; `go test ./internal/telemetry/... -count=1`, guard ADR 0004 y suite global `go test ./... -count=1` PASS. Fuzz de fusión 10 s PASS (500.746 ejecuciones) y fuzz REST PASS (236.843 ejecuciones tras corpus inicial). Benchmark fusión 620,4–676,7 ns/op, 120 B/op y 2 allocs/op; decoder REST 6,00–6,23 µs/op, 2.096 B/op y 23 allocs/op. Vet focal conserva solo los dos warnings `unsafe.Pointer` Win32 heredados; `-race` sigue no disponible porque no hay `gcc` en el entorno.
