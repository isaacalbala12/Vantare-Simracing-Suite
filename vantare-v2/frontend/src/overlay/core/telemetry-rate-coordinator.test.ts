...
describe("createTelemetryRateCoordinator", () => {
  ...
  it("shares a single repaint loop for every subscriber whatever the requested hz", () => {
    ...
    const first = vi.fn();
    const second = vi.fn();
    coordinator.subscribe(undefined, first);
    coordinator.subscribe(undefined, second);
  ...
  it("does not regulate subscriptions without a widget policy", () => {
    const harness = controllableScheduler();
    const coordinator = createTelemetryRateCoordinator({ createScheduler: harness.create });
    const slow = vi.fn();
    const fast = vi.fn();
    coordinator.subscribe(undefined, slow);
    coordinator.subscribe(undefined, fast);

    coordinator.setOverlayFrame(performanceFrame(1, null, {}));
    harness.tick();
    coordinator.setOverlayFrame(performanceFrame(2, null, {}));
    harness.tick();
  ...
  it("applies the global rafCap published by Go", () => {
    ...
    coordinator.setOverlayFrame(performanceFrame(2, 20, { pedals: 20 }));
    harness.tick();
    expect(listener).toHaveBeenCalledTimes(1);
    coordinator.setOverlayFrame(performanceFrame(3, 20, { pedals: 20 }));
    currentTime = 49;
    harness.tick();
    expect(listener).toHaveBeenCalledTimes(1);
    currentTime = 50;
  ...
  it("never paints a widget faster than its widgetHz row", () => {
    ...
    coordinator.setOverlayFrame(performanceFrame(2, 60, { delta: 10 }));
    harness.tick();
    coordinator.setOverlayFrame(performanceFrame(3, 60, { delta: 10 }));
    currentTime = 99;
    harness.tick();
    expect(listener).toHaveBeenCalledTimes(1);
    currentTime = 100;
  ...
  it("keeps level 1 monitor widgets at the existing one-paint-per-frame behavior", () => {
    ...
    coordinator.setOverlayFrame(performanceFrame(2, null, {}));
    harness.tick();
    coordinator.setOverlayFrame(performanceFrame(3, null, {}));
    harness.tick();
    expect(listener).toHaveBeenCalledTimes(2);
    coordinator.dispose();
  });
  ...
  it("repaints once per frame however many snapshots arrived", () => {
    ...
    const coordinator = createTelemetryRateCoordinator({ createScheduler: harness.create });
    const listener = vi.fn();
    coordinator.subscribe(undefined, listener);

    for (let index = 0; index < 5; index += 1) {
      coordinator.setOverlayFrame(performanceFrame(index + 1, null, {}));
    }
    harness.tick();
    expect(listener).toHaveBeenCalledTimes(1);
  ...
  it("stops the repaint loop after the last subscriber unsubscribes", () => {
    ...
    const unsubscribe = coordinator.subscribe(undefined, () => undefined);
    const other = coordinator.subscribe(undefined, () => undefined);
    unsubscribe();
