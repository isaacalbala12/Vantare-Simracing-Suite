...
export function StrategyRecordedWorkflow({ eventId, repositoryVersion, repositoryLoading = false, onRetryRepository, initial, catalog, catalogState, calendar, application, analysis, onExit, onCleanupError, navigation, t }: {
  ...
  useHubSuspendBlocker(`strategy-recorded:${eventId}`, t("strategy.workspace.unsaved"), flow.dirty || flow.busy || formPending || calculationBusy);
  const [libraryOpen, setLibraryOpen] = useState(false);
  const [exitOpen, setExitOpen] = useState(false);
  const discover = () => { setLibraryOpen(true); void flow.sessions.discover(); };
  const edit = (step: RecordedWizardStep) => { if (formPending || calculationBusy) return; flow.change({ ...flow.draft, step }); flow.prepare(); };
  const exit = () => { if (flow.busy || calculationBusy) return; if (flow.dirty || formPending) setExitOpen(true); else onExit(); };
  ...
  const inspectSession = (session: RecordedSession) => {
    if (formPending || calculationBusy) return;
  ...
  const backToPreparation = () => {
    if (flow.busy || formPending || calculationBusy || flow.sessions.corrections.unresolved) return;
    flow.prepare();
  };
  const backBlocked = flow.busy || formPending || calculationBusy || flow.sessions.corrections.unresolved;
  const sourceView = <StrategyRecordedSessionsView controller={{ ...flow.sessions, busy: flow.busy, locked: flow.sessions.locked || formPending || calculationBusy }} onInspect={inspectSession} t={t} />;
  return <div className="strategy-recorded-workflow" data-view={flow.view} data-tab={tab}>
    {navigation?.({ requestExit: exit, draft: flow.draft, view: flow.view, busy: flow.busy || calculationBusy })}
