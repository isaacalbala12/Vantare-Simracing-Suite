...
- La rama exacta
  ...
  commits de implementación; la rama publicada incorpora el expediente
  documental de entrega. La última revisión de implementación y documentación
  validada por CI es `ee008cc00b601c280df7c62c930a6e66f2628f4c`.
...
- `go test -count=20` de Strategy live, app y cmd/vantare, `go test ./...`,
  `go vet` focal, build frontend, `wails3 build DEV=true` y `git diff --check`
  pasan. Los archivos Go del diff están formateados; el chequeo amplio de
  `gofmt` solo encuentra `internal/app/diagnostics_service.go`, deuda heredada
...
- Estado externo: la rama ya está publicada en `origin` y el PR draft `#280`
  apunta a `nightly`. El run CI de esa revisión validada, `32047829206`, pasa
  todos los gates bloqueantes; solo deja avisos advisory de lint global
  heredado. Linear conserva ISA-340 en `In Progress` mientras la revisión esté
  pendiente. No hubo merge, promoción ni release; ISA-153 sigue separado.
