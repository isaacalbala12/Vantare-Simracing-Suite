...
### ISA-1347 — seguimiento post-merge (2026-09-24)

Esta actualización sustituye el estado anterior de PR pendiente. La [PR #1352](https://github.com/isaacalbala12/Vantare-Simracing-Suite/pull/1352) se integró por squash en `nightly` como `f50ab4ab96c4e646d7534d50c804f41087c446db`, verificado en `origin/nightly`. Roadmap digest post-merge PASS; Branch channel gates falló en la suite frontend por el benchmark compacto (1,626 ms / 1,5 ms), run [35931966848](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35931966848). El fallo queda registrado y no se retrocede Nightly.

El seguimiento está en [PR #1354](https://github.com/isaacalbala12/Vantare-Simracing-Suite/pull/1354), rama `vantareapp/isa-1347-nightly-parser-perf` desde el SHA integrado. Los heads `1be6fdf9`, `f7fac47f` y `6de1dd3c` pasaron promoción/ratchet; el gate frontend midió compacto en 1,656, 1,624 y 1,530 ms respectivamente, frente a 1,5 ms. `bae7c02e` reutiliza el buffer UTF-8 con límite exacto de 72 KiB; `264ceb86` normaliza directamente filas compactas comunes sin saltar validación; `c953c82a` evita repetir checks de prototipo tras parseo JSON, conservando el check del request externo. Suite focal del último código 42/42, legacy 0,688 ms y compacto 0,812 ms; typecheck, lint y build PASS. Suite completa del head aprobado: 481 archivos, 4.038 PASS y 2 omitidas; el CI final está registrado abajo.

#### Resultado final del CI — PR #1354

Base Nightly y remoto confirmados en `f50ab4ab96c4e646d7534d50c804f41087c446db`; head `26bd37c942e96e49b6203971e0942d99218909f7`. Promotion path, blocking gates completos (frontend 481 archivos, 4.038 PASS y 2 omitidas; lint modificado; Testing Center visual; build Windows), quality ratchet (`NEW=0`) y GitGuardian aprobaron. El benchmark en CI midió legacy 0,936 ms y compacto 1,156 ms, menor que 1,5 ms. PR #1354 está lista para revisión, sin merge. Nightly sigue en el SHA de #1352 hasta autorización específica de Isaac; la validación física con LMU/Windows/Desktop/OBS permanece pendiente.

La issue [#1347](https://github.com/isaacalbala12/Vantare-Simracing-Suite/issues/1347) sigue abierta en `state:nightly`. La prueba física con LMU activo en Windows y los consumidores Desktop/OBS permanece pendiente; no hay promoción a testers/master ni release. Siguiente paso: revisar #1354 y autorizar su integración si se acepta el seguimiento; después, continuar esa verificación sobre el build integrado.
