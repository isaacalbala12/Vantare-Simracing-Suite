...
describe("recordedCalculationEvent", () => {
  it("maps the exact timed event and preserves zero initial loads and reserves", () => {
    const source = draft({
      ...
      rules: { minPitStops: 1, requiredWindows: [{ fromLap: 10, toLap: 20 }, { fromLap: 30, toLap: 40 }], mandatoryCompounds: ["hard", "wet"], allowedCompoundsByClimate: { dry: ["hard", "wet"], wet: ["soft"] }, driverLimits: { driver: { minLaps: 12, maxLaps: 40, maxContinuousTimeSeconds: 1800, maxTotalTimeSeconds: 5400 } } },
    });
