...
E04/A02 detectó que el importador de arranque declaraba `managed_copy` aunque borraba la copia privada al terminar. En la rama aislada #1373, la declaración pasa a `reference`; Analysis guarda desde una sesión abierta una copia separada en carpeta elegida, verifica tamaño/SHA-256 y conserva el original. Strategy ya ofrece el selector de carpeta en la biblioteca y confirma la copia. El segundo corte permite elegir expresamente un DuckDB fuera de las raíces configuradas, espera estabilidad sin WAL y lo abre con el lector existente. La prueba retira el original y comprueba que la copia conserva ID de sesión, base y revisión inicial de correcciones; si cambia el archivo o está activo, se rechaza. En un borrador con referencias guardadas, Strategy impide adoptar una fuente elegida con otro ID; el camino existente comprueba la revisión exacta cuando coincide. Al cotejar el alcance de #1373 se confirmó que la selección explícita no sustituye el registro privado de la ubicación bajo Analysis ni la recuperación ofrecida sólo si falta el original. Eso, la reapertura tras reinicio con revisión corregida real y QA Wails del selector/copia siguen pendientes; no se declara E04/A02 cerrado. Go completo, 4.306 tests frontend (2 omitidos), typecheck, lint y build pasan en los dos cortes, excepto que el último cambio Go de reloj sólo tiene suite focal. Sin push, PR, CI, merge, promoción ni release.
