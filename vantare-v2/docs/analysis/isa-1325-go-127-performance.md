...
La migración funcionó en los gates obligatorios de Windows, pero el backend JSON predeterminado de Go 1.27.1 introdujo una regresión reproducible en la publicación del overlay. En el primer microbenchmark Windows que activa la demanda del overlay, cada `WriteBatch` de 64 vehículos tardó **39,1 % más** y asignó **34,8 % más bytes** que el mismo árbol compilado con Go 1.25.0. Una contraprueba Windows con `GOEXPERIMENT=nojsonv2` recuperó el tiempo y la memoria del caso. El opt-out temporal se aplicó al candidato y el [pipeline del SHA de código `ac2cc763`](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35850775088) pasó con la configuración exacta del ejecutable. Un experimento posterior de caché de política mejora la ruta JSON nueva, pero no justifica retirar el opt-out; se detalla abajo. El PR continúa en borrador. Estas mediciones no equivalen a latencia del juego ni a un ensayo físico con LMU/OBS.
...
## Caché de política antes de integrar

Isaac aceptó el orden propuesto por Astra max: probar primero la eliminación del JSON repetido de la política y mantener `nojsonv2` hasta demostrar una alternativa suficiente. El commit de producto [`a4596943`](https://github.com/isaacalbala12/Vantare-Simracing-Suite/commit/a45969433c72334258ffeb427e80801371af1b7a) conserva la representación V2 de `WidgetHz` y `RafCap` por `performanceRevision`, y suministra el `SourceHz` observado a cada proyección. La prueba de contrato comprueba tasas numéricas y de evento después de reutilizar la caché y cambiar la política.

El [run Windows 35882663551](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35882663551) comparó el código anterior `9b42d74f` con `a4596943`, sobre Go 1.27.1 en un solo runner AMD EPYC 9V74. El commit temporal `8cd71027` solo añadió el workflow de ensayo. Se compilaron cuatro ejecutables de test: original y caché, cada uno con JSON predeterminado y con `nojsonv2`. El [script versionado](../../scripts/bench/telemetry-microbench.ps1) ejecutó diez pares alternos por backend con `GOMAXPROCS=1`, `-test.cpu=1`, `-test.benchtime=100ms` y `-test.benchmem`. [Muestras, hashes y resúmenes preservados](isa-1325-go-127-performance-data/windows-policy-cache/).

| Backend Go 1.27.1 · `WriteBatch` 64 vehículos | Anterior | Con caché | Cambio de tiempo | Bytes/op | Allocs/op |
| --- | ---: | ---: | ---: | ---: | ---: |
| JSON predeterminado | 463,2 µs | 385,0 µs | **−16,9 %** (`p<0,001`; 9/10 pares más rápidos) | 406,7 → 297,5 KiB (**−26,8 %**) | 404 → 273 (**−32,4 %**) |
| `GOEXPERIMENT=nojsonv2` | 314,9 µs | 309,5 µs | **Sin diferencia concluyente** (`p=0,190`; 7/10 pares más rápidos) | 302,1 → 299,3 KiB (**−0,9 %**) | 373 → 300 (**−19,6 %**) |

Los datos apoyan conservar la caché: reduce asignaciones en ambos backends y mejora claramente la publicación con JSON nuevo. El fuerte descenso de bytes con JSON nuevo es compatible con la hipótesis de reutilización de buffers planteada por Astra, pero este A/B no demuestra ese mecanismo; haría falta un perfil específico de esa ruta en Windows. Con caché, la mediana de JSON predeterminado sigue siendo 385,0 µs frente a 309,5 µs con `nojsonv2`; esas dos variantes se midieron en secuencias separadas, por lo que la tabla no calcula significación estadística entre ellas. **Se mantiene el opt-out.** El siguiente experimento de VAN-756 será medir un buffer propio con JSON v2; una caché por secciones vendría después, y diferir a `Pull` solo si se mide desperdicio suficiente y latencia aceptable. El A/B compara el código previo al merge posterior de `nightly@b725c402`; ese merge conserva la implementación del overlay, pero su árbol completo requiere los checks del HEAD final.

## Límite y siguiente decisión

Los tres runs son microbenchmarks de runner compartido con fixture generado. Diez pares alternos reducen el sesgo por cambios lentos del entorno, pero no sustituyen una prueba del ejecutable real durante una sesión LMU/OBS. El benchmark de publicación registra demanda, pero no consume mediante `Pull`, socket ni SSE. No hay un presupuesto de latencia o asignaciones de producto aprobado en VAN-747. En `ac2cc763`, [promoción/roadmap y gate Windows](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35850775088) terminaron **SUCCESS**: contratos, suite Go y frontend, gate visual, build Wails y verificación de `GOEXPERIMENT=nojsonv2` en el EXE. [Quality](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35850775070) devolvió **FAILURE / REVIEW_REQUIRED** por la política modificada; en la pasada CI final todos los analizadores registraron `NEW=0` y `MOVED=0`, y Knip resolvió una incidencia. GitGuardian terminó **SUCCESS**. El subagente Astra max revisó mitigación, scripts de medida e informe: ACCEPT con reservas metodológicas y sin hallazgos P0–P2. Toda modificación posterior del PR requiere verificar sus checks en el nuevo HEAD. Nightly y release continúan sin cambios.
