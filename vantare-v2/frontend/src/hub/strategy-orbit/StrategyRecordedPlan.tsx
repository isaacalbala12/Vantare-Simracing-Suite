...
function CalculationStages({ state, manual, t }: { readonly state: "idle" | "running" | "blocked"; readonly manual: boolean; readonly t: (key: string) => string }) {
  const current = state === "idle" ? 0 : state === "running" ? 1 : -1;
  return <ol className="strategy-recorded-plan__stages">
    {[t(manual ? "strategy.workspace.manualReferences" : "strategy.workspace.sources"), t("strategy.workspace.validationPending"), t("strategy.workspace.plan")].map((label, index) => <li key={label} data-state={state === "blocked" && index > 0 ? "blocked" : index < current ? "done" : index === current ? "current" : "next"}><span>{index < current ? "✓" : index + 1}</span><strong>{label}</strong></li>)}
  </ol>;
}
...
export function StrategyRecordedPlan({ draft, state, acceptance, sourceLabels = {}, locked, onChange, onCalculate, onRecalculateStints, onRecalculatePits, onCancel, t }: {
  ...
  const editors = plan && state.status === "success" ? <nav className="strategy-recorded-plan__editors" aria-label={t("strategy.stint.eyebrow")}>
    <button type="button" aria-label={t("strategy.stint.title")} aria-current={editor === "stints" ? "step" : undefined} disabled={stintDirty || pitDirty || locked || accepting} onClick={() => setEditor("stints")}><span>{t("strategy.stint.eyebrow")}</span><strong>{t("strategy.stint.title")}</strong><b className="strategy-recorded-plan__editors-short" aria-hidden="true">{t("strategy.plan.stint")}</b><small>{t("strategy.stint.hint")}</small></button>
    {plan.stopDetails.length ? <button type="button" aria-label={t("strategy.pitEdit.title")} aria-current={editor === "pits" ? "step" : undefined} disabled={stintDirty || pitDirty || locked || accepting} onClick={() => setEditor("pits")}><span>{t("strategy.pitEdit.eyebrow")}</span><strong>{t("strategy.pitEdit.title")}</strong><b className="strategy-recorded-plan__editors-short" aria-hidden="true">{t("strategy.plan.stops")}</b><small>{t("strategy.pitEdit.hint")}</small></button> : null}
  </nav> : null;
  const backToPlan = editor !== "plan" ? <div className="strategy-recorded-plan__editor-head"><Button variant="ghost" disabled={!!plan && (stintDirty || pitDirty || locked || accepting)} onClick={() => { setEditor("plan"); setStintDirty(false); setPitDirty(false); }}>← {t("strategy.data.tab.plan")}</Button>{state.status === "error" ? <Button variant="primary" disabled={locked || accepting || !draft.calculationMode} onClick={calculate}>{t("strategy.workspace.recalculateBase")}</Button> : null}</div> : null;
  const pendingCourse = <div className="strategy-recorded-plan__course" aria-label={t("strategy.workspace.raceDistance")}>
