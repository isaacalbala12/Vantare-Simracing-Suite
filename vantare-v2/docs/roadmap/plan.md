...
- cuerpo: El candidato de CI prepara GTK4 y WebKitGTK 6.0 en los dos jobs Linux para que govet y deadcode analicen el grafo Wails declarado sin recortar paquetes. Los fallos de instalación, integridad y hallazgos NEW siguen cerrando el gate; no cambia baselines. La infraestructura está integrada en Nightly mediante PR #1297. La reparación de la prueba negativa de cambios de política usa un repositorio Git temporal y sigue en revisión separada; no modifica los analizadores ni sus reglas. El detector de duplicación distingue reagrupaciones mediante igualdad íntegra de las fuentes respecto a la procedencia aceptada y la base de la PR; conserva el bloqueo de copias nuevas y la revisión obligatoria de la política.
- cuerpo.en: The CI candidate prepares GTK4 and WebKitGTK 6.0 in both Linux jobs so govet and deadcode can analyse the declared Wails graph without dropping packages. Installation failures, integrity errors and NEW findings still fail the gate; baselines remain unchanged. The infrastructure is integrated into Nightly through PR #1297. A separate candidate repairs the policy-change negative test using a temporary Git repository; analyzers and their rules remain unchanged. The duplication gate distinguishes regrouping through complete source equality against accepted provenance and the PR base; new copies remain blocking and policy changes still require review.
- cuerpo.pt: O candidato de CI prepara GTK4 e WebKitGTK 6.0 nos dois jobs Linux para que govet e deadcode analisem o grafo Wails declarado sem excluir pacotes. Falhas de instalação, erros de integridade e achados NEW continuam a fechar o gate; os baselines não mudam. A infraestrutura está integrada no Nightly pela PR #1297. Um candidato separado corrige o teste negativo de mudanças de política usando um repositório Git temporário; os analisadores e suas regras permanecem inalterados. O detector distingue reagrupamentos pela igualdade integral das fontes com a procedência aceita e a base da PR; novas cópias continuam bloqueantes e alterações da política exigem revisão.
- cuerpo.it: Il candidato CI prepara GTK4 e WebKitGTK 6.0 in entrambi i job Linux affinché govet e deadcode analizzino il grafo Wails dichiarato senza escludere pacchetti. Errori di installazione, integrità e rilievi NEW continuano a bloccare il gate; i baseline restano invariati. L’infrastruttura è integrata in Nightly tramite PR #1297. Un candidato separato corregge il test negativo delle modifiche alla policy usando un repository Git temporaneo; analizzatori e regole restano invariati. Il controllo distingue i raggruppamenti tramite identità completa delle sorgenti rispetto alla provenienza accettata e alla base della PR; nuove copie restano bloccanti e le modifiche alla policy richiedono revisione.
- etiqueta: Candidato
...
- cuerpo: Spotter y las familias fuel, sanciones, vueltas, tiempos y boxes consumen telemetría canónica y entregan avisos priorizados por el mismo bus, con rollback exclusivo del stack anterior. El widget engineer-radio del overlay muestra cada aviso publicado, incluidas las respuestas del carril de voz experimental y los avisos de rendimiento. Orbit guarda módulos, sensibilidad y salidas, y avisa de forma contextual cuando faltan datos espaciales para Spotter. Timings guarda silencio fuera de carrera, en boxes y en los últimos dos minutos de una carrera cronometrada, e invalida avisos pendientes si pierde ese contexto. La orden automática de parar por combustible queda desactivada hasta demostrar su necesidad; se conservan los avisos de litros y autonomía. Spotter admite solapes nuevos solo con diferencias de velocidad inferiores a 12 m/s por eje horizontal del mundo y hace elegible el aviso libre después de 150 ms desde el primer vacío. Revalida la ocupación observada antes de iniciar avisos y filtra de nuevo las reapariciones. El audio Windows comienza al abrirse el medio y finaliza por señal del reproductor, sin pausa fija ni cola calculada por duración; los errores se comunican y la reproducción sigue siendo cancelable. La paridad acústica con CrewChief requiere validación física. La paridad completa con CrewChief y la validación física siguen pendientes. La pantalla del ingeniero permite probar el reproductor real con el ingeniero desactivado, confirma cambios guardados y muestra las últimas 200 entregas con salida configurada y resultado visual/audio. El diagnóstico se puede revisar y exportar localmente; la falta de audio cacheado queda explícita. Las rutas de audio inexistentes o no válidas se rechazan antes de abrir el reproductor, sin interrumpir el sonido que ya esté reproduciéndose.
- cuerpo.en: Spotter plus fuel, penalties, laps, timings and pit families consume canonical telemetry and deliver prioritized alerts through the same bus, with an exclusive rollback to the previous stack. The overlay engineer-radio widget shows every published alert, including experimental voice-lane answers and performance notices. Orbit saves modules, sensitivity and outputs, and shows a contextual warning when Spotter lacks spatial data. Timings stays silent outside races, in the pits and during the final two minutes of a timed race, and invalidates pending reports when that context is lost. Automatic fuel pit orders are disabled until their need can be demonstrated; fuel-level and range reports remain available. Spotter admits new overlaps only when world horizontal velocity differences are below 12 m/s per axis and makes clear reports eligible after 150 ms from the first empty observation. It revalidates observed occupancy before reports start and filters reappearing overlaps again. Windows audio starts when the media opens and finishes on the player signal, without a fixed pause or a duration-based tail; errors are reported and playback remains cancellable. Acoustic parity with CrewChief requires physical validation. Full CrewChief parity and physical validation remain pending. The engineer screen tests the actual player while the engineer is disabled, confirms saved changes and shows the last 200 deliveries with configured output and visual/audio results. Diagnostics can be previewed and exported locally; missing cached audio is explicit. Missing or invalid audio paths are rejected before opening the player, without interrupting audio already playing.
- cuerpo.pt: Spotter e as famílias de combustível, penalizações, voltas, tempos e boxes consomem telemetria canónica e entregam alertas priorizados pelo mesmo bus, com rollback exclusivo para o stack anterior. O widget engineer-radio do overlay mostra cada alerta publicado, incluindo respostas do carril de voz experimental e avisos de desempenho. O Orbit guarda módulos, sensibilidade e saídas, e avisa quando faltam dados espaciais para o Spotter. Timings permanece em silêncio fora da corrida, nas boxes e nos últimos dois minutos de uma corrida por tempo, e invalida avisos pendentes quando perde esse contexto. A ordem automática de parar por combustível fica desativada até se demonstrar a necessidade; mantêm-se os avisos de litros e autonomia. O Spotter admite novos emparelhamentos apenas com diferenças de velocidade inferiores a 12 m/s por eixo horizontal do mundo e permite o aviso de lado livre após 150 ms desde a primeira observação livre. Revalida a ocupação observada antes de iniciar os avisos e volta a filtrar as reaparições. O áudio Windows começa quando o ficheiro abre e termina por sinal do reprodutor, sem pausa fixa nem espera calculada pela duração; os erros são comunicados e a reprodução continua cancelável. A paridade acústica com CrewChief requer validação física. A paridade completa com CrewChief e a validação física continuam pendentes. A tela do engenheiro testa o reprodutor real com o engenheiro desativado, confirma alterações salvas e mostra os últimos 200 envios com saída configurada e resultado visual/áudio. O diagnóstico pode ser revisado e exportado localmente; a falta de áudio em cache fica explícita. Caminhos de áudio inexistentes ou inválidos são rejeitados antes de abrir o reprodutor, sem interromper o áudio já em reprodução.
- cuerpo.it: Spotter e le famiglie carburante, penalità, giri, tempi e box consumano telemetria canonica e consegnano avvisi prioritari sullo stesso bus, con rollback esclusivo allo stack precedente. Il widget engineer-radio dell'overlay mostra ogni avviso pubblicato, comprese le risposte del canale vocale sperimentale e gli avvisi di prestazioni. Orbit salva moduli, sensibilità e uscite e avvisa quando mancano dati spaziali per lo Spotter. Timings resta in silenzio fuori gara, ai box e negli ultimi due minuti di una gara a tempo, e invalida gli avvisi in attesa quando perde questo contesto. L’ordine automatico di fermarsi per il carburante è disattivato finché non se ne dimostra la necessità; restano gli avvisi su litri e autonomia. Lo Spotter ammette nuovi affiancamenti solo con differenze di velocità inferiori a 12 m/s per asse orizzontale del mondo e abilita l’avviso di lato libero dopo 150 ms dalla prima osservazione libera. Rivalida l’occupazione osservata prima di avviare gli avvisi e filtra nuovamente gli affiancamenti ricomparsi. L’audio Windows inizia quando si apre il file e termina al segnale del lettore, senza pausa fissa né attesa calcolata dalla durata; gli errori vengono segnalati e la riproduzione resta annullabile. La parità acustica con CrewChief richiede verifica fisica. La parità completa con CrewChief e la verifica fisica restano da completare. La schermata dell’ingegnere prova il riproduttore reale con l’ingegnere disattivato, conferma le modifiche salvate e mostra gli ultimi 200 invii con uscita configurata e risultato visivo/audio. La diagnostica si può esaminare ed esportare localmente; l’assenza di audio in cache è esplicita. I percorsi audio mancanti o non validi vengono rifiutati prima di aprire il lettore, senza interrompere l’audio già in riproduzione.
- etiqueta: En revisión
- etiqueta.en: In review
- etiqueta.pt: Em revisão
- etiqueta.it: In revisione
...
- cuerpo: El planificador usa una única autoridad de cálculo y guardado sobre las sesiones DuckDB reales de LMU. La entrada asistida parte de las carreras del calendario, cruza sede y clase mediante identidades canónicas declaradas en Go y solo pide trazado cuando el histórico contiene varios; después Orbit aplica inputs y clima, muestra backtests neutrales, consume referencias firmadas solo de confianza aprobada y para la combinación elegida e importa el histórico local con progreso. El perfil único con límites se aplica antes de optimizar y el solver ya puede respetar una secuencia multipiloto opcional en solve y replay. Las secuencias largas con perfiles equivalentes reutilizan el atajo escalar existente sin elevar el presupuesto ni cambiar la búsqueda general. En carreras por vueltas, CalculateOrbit ya distingue la rotación fijada de la selección libre entre candidatos. T03a distingue cancelación de plazo agotado, T03b evita colisiones de comandos entre montajes, T03c espera las entradas derivadas cuando hay telemetría seleccionada y T03d rechaza optimalidad desconocida y T03e distingue presupuesto agotado de inviabilidad con clima. T03a–e cierra el contrato actual sin fabricar estados; el selector recorded ya conserva el criterio fijo o libre y la secuencia exacta; el nuevo query read-only prepara conjuntamente las revisiones exactas del borrador sin crear otro evento ni combinar telemetría en React; la disponibilidad horaria, el clima individual, el plan parcial y la optimalidad final siguen pendientes en sus cortes posteriores. El catálogo permanece vacío sin confianza aprobada. Las nuevas revisiones Orbit custodian la entrada exacta del cálculo vigente; cambiar una referencia de fuente impide reconocer la revisión anterior como guardada. El bridge Go ya abre una revisión Orbit por su referencia completa sin consultar la fuente ni sustituirla por la más reciente. El cliente TypeScript transporta esa referencia y rechaza una revisión válida ausente o distinta. La biblioteca publica además todas las referencias históricas sin cargar sus payloads, por lo que A sigue siendo descubrible después de crear B. Mis estrategias muestra los planes guardados aunque ya no tengan borrador y permite elegir y consultar cada resultado histórico exacto sin reabrir su fuente. T15a2c #1269 resuelve la selección libre temporal con el reloj exacto de SolverV2, incluidas formación y paradas, sin estimar vueltas por ritmo medio. T15b #1270 conecta el panel Plan: conserva una condición seca o mojada explícita, prepara las revisiones exactas, valida las familias necesarias y permite calcular o cancelar sin guardar ni aceptar automáticamente; las respuestas antiguas se descartan. T15c #1271 presenta cobertura parcial o el plan exacto de SolverV2 con estados honestos, Fuel/VE, fuentes, versión y criterio de optimalidad; aceptar conserva la petición y el resultado exactos mediante la recuperación duradera de revisiones y sigue separado de guardar la configuración. T16a #1273 reutiliza la secuencia de pilotos y los overrides de vueltas para comparar la propuesta exacta con una variante de stint restringida en el mismo SolverV2. T16b #1274 conecta piloto, límites de stint equivalentes por arrastre/teclado, obsolescencia, recálculo exacto y coste frente a la base. T17a #1275 fija por parada Fuel/VE añadidos y neumáticos físicos en el replay único, con el modo paralelo/secuencial gobernado por el evento. T17b #1276 conecta la edición productiva de esos servicios, el desglose, la obsolescencia, el recálculo sin nueva preparación y el coste exacto. T18 #1277 cierra localmente la paridad A4; pass-28-computer-use suma 22 estados principales y 72 variantes responsive ES/EN/PT/IT, mantiene la base por encima de 9/10, corrige la lista integrada de sesiones y conserva salida y reintento tras una parada inviable. El preflight nativo sustituye el drawer de fuentes por una pantalla A4 completa y repite automáticamente la observación segura que libera los DuckDB estables; queda T22 para completar la aceptación nativa. ISA-1314 sustituye localmente el recorrido inicial A4 por el menú y la mesa v5 aprobados; la nueva entrada manual reutiliza el motor existente. La evidencia A4 queda como antecedente y no certifica la nueva interfaz. El corte local #1395 ya aplica ventanas temporales exactas por piloto en SolverV2; queda el horario legacy. #1396 permite editar minutos de carrera por piloto y transporta exactamente sus ventanas al cálculo; el horario legacy por reloj sigue pendiente.
- cuerpo.en: The planner uses one calculation and storage authority over real LMU DuckDB sessions. Assisted entry starts from calendar races, joins venue and class through canonical identities declared in Go, and asks for a layout only when history contains several; Orbit then applies inputs and weather, shows neutral backtests, consumes signed references only with approved trust and for the selected combination, and imports local history with progress. A constrained single-driver profile is applied before optimization, and the solver can now preserve an optional multi-driver sequence in solve and replay. Long sequences with equivalent profiles reuse the existing scalar shortcut without raising the budget or changing the general search. For lap races, CalculateOrbit now distinguishes fixed rotation from free candidate selection. T03a distinguishes cancellation from an expired deadline, T03b prevents command collisions across mounts, T03c waits for derived inputs when telemetry is selected, and T03d rejects unknown optimality and T03e distinguishes exhausted budget from weather infeasibility. T03a–e closes the current contract without fabricating states; the recorded selector now preserves the fixed or free criterion and exact sequence; the new read-only query jointly prepares the draft exact revisions without creating another event or combining telemetry in React; time-based availability, individual weather, a partial plan, and final optimality remain pending in later slices. The catalog stays empty without approved trust. New Orbit revisions retain the exact input of the current calculation; changing a source reference prevents the previous revision from being recognized as saved. The Go bridge now opens an Orbit revision by its complete reference without querying the source or replacing it with the latest revision. The TypeScript client carries that reference and rejects a valid response when its revision is missing or different. The library also publishes every historical reference without loading payloads, so A remains discoverable after B is created. My strategies shows saved plans even when they no longer have a draft and lets the person choose and inspect each exact historical result without reopening its source. T15a2c #1269 resolves timed free selection with SolverV2's exact clock, including formation and pit stops, without estimating laps from average pace. T15b #1270 connects the Plan panel: it retains an explicit dry or wet condition, prepares the exact revisions, validates required families, and supports calculate or cancel without saving or accepting automatically; stale responses are discarded. T15c #1271 presents partial coverage or the exact SolverV2 plan with honest states, Fuel/VE, sources, version, and optimality criterion; acceptance preserves the exact request and result through durable revision recovery and remains separate from saving configuration. T16a #1273 reuses the driver sequence and lap overrides to compare the exact proposal with a constrained stint variant in the same SolverV2. T16b #1274 connects driver, equivalent drag/keyboard stint boundaries, staleness, exact recalculation, and cost against the base. T17a #1275 fixes added Fuel/VE and physical tyres per stop in the single replay, with parallel/sequential mode governed by the event. T17b #1276 connects productive service editing, breakdown, staleness, recalculation without new preparation, and exact cost. T18 #1277 locally closes A4 parity; pass-28-computer-use adds 22 primary states and 72 ES/EN/PT/IT responsive variants, keeps the visual baseline above 9/10, fixes the embedded session list, and preserves exit and retry after an infeasible pit edit. The native preflight replaces the sources drawer with a full A4 screen and automatically repeats the safe observation that releases stable DuckDB files; T22 remains for complete native acceptance. ISA-1314 locally replaces the A4 entry flow with the approved v5 menu and desk; manual entry reuses the existing engine. A4 evidence is historical and does not certify the new interface. Local #1395 now applies exact per-driver time windows in SolverV2; legacy clock-time mapping remains open. #1396 lets drivers edit elapsed race minutes and carries their exact windows into calculation; legacy wall-clock mapping remains pending.
- cuerpo.pt: O planeador usa uma única autoridade de cálculo e gravação sobre as sessões DuckDB reais do LMU. A entrada assistida começa nas corridas do calendário, cruza circuito e classe através de identidades canónicas declaradas em Go e só pede o traçado quando o histórico contém vários; depois o Orbit aplica inputs e meteorologia, mostra backtests neutros, consome referências assinadas apenas com confiança aprovada e para a combinação escolhida e importa o histórico local com progresso. O perfil de piloto único com limites é aplicado antes da otimização e o solver já pode respeitar uma sequência multipiloto opcional no cálculo e replay. Sequências longas com perfis equivalentes reutilizam o atalho escalar existente sem aumentar o orçamento nem alterar a busca geral. Nas corridas por voltas, CalculateOrbit já distingue a rotação fixa da seleção livre entre candidatos. T03a distingue cancelamento de prazo esgotado, T03b evita colisões de comandos entre montagens, T03c espera os inputs derivados quando há telemetria selecionada e T03d rejeita optimalidade desconhecida e T03e distingue orçamento esgotado de inviabilidade meteorológica. T03a–e fecha o contrato atual sem inventar estados; o seletor recorded já conserva o critério fixo ou livre e a sequência exata; a nova consulta read-only prepara conjuntamente as revisões exatas do rascunho sem criar outro evento nem combinar telemetria no React; a disponibilidade horária, a meteorologia individual, o plano parcial e a optimalidade final continuam pendentes nos cortes posteriores. O catálogo permanece vazio sem confiança aprovada. As novas revisões Orbit guardam a entrada exata do cálculo atual; alterar uma referência de origem impede reconhecer a revisão anterior como guardada. O bridge Go já abre uma revisão Orbit pela referência completa sem consultar a origem nem substituí-la pela revisão mais recente. O cliente TypeScript transporta essa referência e rejeita uma revisão válida ausente ou diferente. A biblioteca também publica todas as referências históricas sem carregar payloads, mantendo A acessível após criar B. Minhas estratégias mostra os planos guardados mesmo sem rascunho e permite escolher e consultar cada resultado histórico exato sem reabrir a origem. T15a2c #1269 resolve a seleção livre temporal com o relógio exato de SolverV2, incluindo formação e paragens, sem estimar voltas pelo ritmo médio. T15b #1270 liga o painel Plano: conserva uma condição seca ou molhada explícita, prepara as revisões exatas, valida as famílias necessárias e permite calcular ou cancelar sem guardar nem aceitar automaticamente; as respostas antigas são descartadas. T15c #1271 apresenta cobertura parcial ou o plano exato do SolverV2 com estados honestos, Fuel/VE, fontes, versão e critério de otimalidade; aceitar conserva o pedido e o resultado exatos através da recuperação durável de revisões e continua separado de guardar a configuração. T16a #1273 reutiliza a sequência de pilotos e os overrides de voltas para comparar a proposta exata com uma variante de stint restringida no mesmo SolverV2. T16b #1274 liga piloto, limites de stint equivalentes por arrasto/teclado, obsolescência, recálculo exato e custo face à base. T17a #1275 fixa Fuel/VE adicionados e pneus físicos por paragem no replay único, com o modo paralelo/sequencial governado pelo evento. T17b #1276 liga a edição produtiva dos serviços, o detalhe, a obsolescência, o recálculo sem nova preparação e o custo exato. T18 #1277 fecha localmente a paridade A4; pass-28-computer-use soma 22 estados principais e 72 variantes responsive ES/EN/PT/IT, mantém a base visual acima de 9/10, corrige a lista integrada de sessões e conserva saída e nova tentativa após uma paragem inviável. O preflight nativo substitui o drawer de fontes por um ecrã A4 completo e repete automaticamente a observação segura que liberta DuckDB estáveis; resta T22 para completar a aceitação nativa. ISA-1314 substitui localmente a entrada A4 pelo menu e mesa v5 aprovados; a entrada manual reutiliza o motor existente. A evidência A4 é histórica e não certifica a nova interface. O corte local #1395 já aplica janelas temporais exatas por piloto no SolverV2; falta o horario legacy. #1396 permite editar minutos de corrida por piloto e leva as janelas exatas ao cálculo; o horário legacy continua pendente.
- cuerpo.it: Il pianificatore usa un'unica autorità di calcolo e salvataggio sulle sessioni DuckDB reali di LMU. L'ingresso assistito parte dalle gare del calendario, collega sede e classe tramite identità canoniche dichiarate in Go e chiede il tracciato solo quando lo storico ne contiene più di uno; Orbit applica poi input e meteo, mostra backtest neutrali, usa riferimenti firmati solo con fiducia approvata e per la combinazione scelta e importa lo storico locale con avanzamento. Il profilo singolo con limiti viene applicato prima dell'ottimizzazione e il solver può ora rispettare una sequenza multipilota opzionale nel calcolo e nel replay. Le sequenze lunghe con profili equivalenti riutilizzano la scorciatoia scalare esistente senza aumentare il budget né modificare la ricerca generale. Nelle gare a giri, CalculateOrbit ora distingue la rotazione fissa dalla selezione libera tra candidati. T03a distingue l’annullamento dalla scadenza, T03b evita collisioni dei comandi tra montaggi, T03c attende gli input derivati quando è selezionata la telemetria e T03d rifiuta optimality sconosciuta e T03e distingue budget esaurito da impossibilità meteorologica. T03a–e chiude il contratto attuale senza inventare stati; il selettore recorded ora conserva il criterio fisso o libero e la sequenza esatta; la nuova query read-only prepara congiuntamente le revisioni esatte della bozza senza creare un altro evento né combinare la telemetria in React; la disponibilità oraria, il meteo individuale, il piano parziale e l’optimalità finale restano nei tagli successivi. Il catalogo resta vuoto senza fiducia approvata. Le nuove revisioni Orbit conservano l’input esatto del calcolo corrente; modificare un riferimento della fonte impedisce di riconoscere la revisione precedente come salvata. Il bridge Go ora apre una revisione Orbit tramite il riferimento completo senza consultare la fonte né sostituirla con la revisione più recente. Il client TypeScript trasporta quel riferimento e rifiuta una revisione valida assente o diversa. La libreria pubblica inoltre tutti i riferimenti storici senza caricare i payload, così A resta individuabile dopo la creazione di B. Le mie strategie mostra i piani salvati anche senza bozza e permette di scegliere e consultare ogni risultato storico esatto senza riaprire la fonte. T15a2c #1269 risolve la selezione libera a tempo con il cronometro esatto di SolverV2, includendo formazione e soste, senza stimare i giri dal passo medio. T15b #1270 collega il pannello Piano: conserva una condizione asciutta o bagnata esplicita, prepara le revisioni esatte, convalida le famiglie necessarie e consente di calcolare o annullare senza salvare né accettare automaticamente; le risposte obsolete vengono scartate. T15c #1271 presenta la copertura parziale o il piano esatto di SolverV2 con stati corretti, Fuel/VE, fonti, versione e criterio di ottimalità; l’accettazione conserva richiesta e risultato esatti tramite il recupero durevole delle revisioni e resta separata dal salvataggio della configurazione. T16a #1273 riutilizza la sequenza dei piloti e gli override dei giri per confrontare la proposta esatta con una variante di stint vincolata nello stesso SolverV2. T16b #1274 collega pilota, limiti di stint equivalenti tramite trascinamento/tastiera, obsolescenza, ricalcolo esatto e costo rispetto alla base. T17a #1275 fissa Fuel/VE aggiunti e pneumatici fisici per sosta nel replay unico, con la modalità parallela/sequenziale governata dall’evento. T17b #1276 collega la modifica produttiva dei servizi, il dettaglio, lo stato obsoleto, il ricalcolo senza nuova preparazione e il costo esatto. T18 #1277 chiude localmente la parità A4; pass-28-computer-use aggiunge 22 stati principali e 72 varianti responsive ES/EN/PT/IT, mantiene la base visiva sopra 9/10, corregge la lista sessioni integrata e conserva uscita e nuovo tentativo dopo una sosta non fattibile. Il preflight nativo sostituisce il drawer delle fonti con una schermata A4 completa e ripete automaticamente l’osservazione sicura che libera i DuckDB stabili; resta T22 per completare l’accettazione nativa. ISA-1314 sostituisce localmente l'ingresso A4 con menu e tavolo v5 approvati; l'ingresso manuale riusa il motore esistente. L'evidenza A4 è storica e non certifica la nuova interfaccia. Il lavoro locale #1395 applica già finestre temporali esatte per pilota in SolverV2; resta l'orario legacy. #1396 permette di modificare i minuti di gara per pilota e passa le finestre esatte al calcolo; resta l’orario legacy.
- etiqueta: Feature
...
- cuerpo: El catálogo autorizado conserva un backup validado y permite recuperar el archivo principal ausente o corrupto, guardando el dañado para revisión. Distingue recuperación, corrupción y escritura incierta. El estado de importación también se recupera y Strategy muestra causas de indisponibilidad y avisos de recuperación. El progreso se reconcilia con las sesiones autorizadas conservadas: detecta pérdidas reintentables y evita importaciones duplicadas. Cada importación tiene un deadline real en el backend y muestra los vencimientos reintentables; descarta resultados tardíos. Queda pendiente la validación visual en Wails.
- cuerpo.en: The authorized catalog keeps a validated backup and can restore a missing or corrupt primary while preserving damaged data for review. It distinguishes recovery, corruption and uncertain writes. Import state also recovers, and Strategy displays unavailability causes and recovery notices. Progress reconciles with retained authorized sessions, reports retryable losses and avoids duplicate imports. Each import has a backend deadline, reports retryable timeouts and discards late results. Visual Wails validation remains pending.
- cuerpo.pt: O catálogo autorizado mantém um backup validado e recupera o ficheiro principal ausente ou corrompido, preservando os dados danificados. Distingue recuperação, corrupção e escrita incerta. O estado de importação também é recuperado e Strategy apresenta causas de indisponibilidade e avisos de recuperação. O progresso é reconciliado com as sessões autorizadas conservadas, identifica perdas recuperáveis e evita importações duplicadas. Cada importação tem um prazo no backend, apresenta tempos limite recuperáveis e descarta resultados tardios. Falta a validação visual em Wails.
...
- tipo: feature
- titulo.en: Strategy launch menu and recorded editor
- titulo.pt: Menu de estratégia e editor de telemetria
- titulo.it: Menu strategia ed editor della telemetria
- cuerpo: La entrega local ISA-1331 presenta un menú de ancho completo para comenzar desde telemetría o referencias manuales y muestra a la derecha borradores y planes guardados. Preparación consulta las revisiones exactas para mostrar ritmo, Fuel y energía virtual disponibles, con procedencia, estados de ausencia y aviso explícito para observaciones inciertas que no se usan para calcular; el circuito usa geometría LMU identificada o indica que no hay trazado disponible. Carrera, Datos, Plan, Revisiones, Stint, Parada, biblioteca e historial comparten la mesa v5 en grises Orbit, sin animar toda la interfaz al entrar. Se conservan búsqueda y selección de hasta cuatro sesiones, originales intactos, correcciones, cálculo Go, cancelación, cobertura parcial, edición y aceptación separada del guardado. La validación nativa y visual integral con DuckDB reales y aceptación de Isaac siguen pendientes; el HTML y los tests no cierran T22. También quedan pendientes preparación de resistencia con memoria acotada (#1375), calibración independiente y validación horaria nativa. Live y la investigación OSS/Monte Carlo permanecen aplazados. Rama aislada, sin integración ni publicación. El corte local #1395 ya aplica ventanas temporales exactas por piloto en SolverV2; la mesa #1396 ya permite editarlas en minutos de carrera, guardarlas y enviarlas al cálculo.
- cuerpo.en: Local ISA-1331 delivery provides a full-width launch menu for telemetry or manual references, with saved drafts and plans on the right. Preparation reads exact revisions for available pace, Fuel and virtual energy, showing provenance, missing-data states and explicit warnings for uncertain observations excluded from calculation; the circuit uses identified LMU geometry or explicitly reports no available outline. Race, Data, Plan, Revisions, Stint, Pit, library and history share the grayscale Orbit v5 desk without animating the whole interface on entry. Search and up to four selected sessions, intact originals, corrections, Go calculation, cancellation, partial coverage, editing and acceptance separate from saving remain available. Complete native and visual validation with real DuckDB files and Isaac's acceptance are pending; HTML and tests do not close T22. Bounded-memory preparation for endurance (#1375), independent calibration and native time-window validation also remain pending. Live and OSS/Monte Carlo research stay deferred. Isolated branch, without integration or publication. Local #1395 now applies exact per-driver time windows in SolverV2; the #1396 desk now edits these windows in race minutes, saves them and sends them to calculation.
- cuerpo.pt: A entrega local ISA-1331 apresenta um menu de largura total para começar por telemetria ou referências manuais, com rascunhos e planos guardados à direita. A preparação lê revisões exatas para mostrar ritmo, Fuel e energia virtual disponíveis, com proveniência, estados de ausência e aviso explícito para observações incertas que não entram no cálculo; o circuito usa geometria LMU identificada ou indica que não há traçado disponível. Corrida, Dados, Plano, Revisões, Stint, Paragem, biblioteca e histórico partilham a mesa v5 em cinzentos Orbit, sem animar toda a interface ao entrar. Mantêm-se pesquisa e seleção de até quatro sessões, originais intactos, correções, cálculo Go, cancelamento, cobertura parcial, edição e aceitação separada da gravação. Faltam validação nativa e visual integral com DuckDB reais e aceitação de Isaac; HTML e testes não encerram T22. Também faltam preparação de resistência com memória limitada (#1375), calibração independente e validação temporal nativa. Live e investigação OSS/Monte Carlo continuam adiados. Ramo isolado, sem integração nem publicação. O corte local #1395 já aplica janelas temporais exatas por piloto no SolverV2; a mesa #1396 já permite editá-las em minutos de corrida, guardá-las e enviá-las ao cálculo.
- cuerpo.it: La consegna locale ISA-1331 offre un menu a tutta larghezza per iniziare da telemetria o riferimenti manuali, con bozze e piani salvati a destra. La preparazione legge revisioni esatte per mostrare ritmo, Fuel ed energia virtuale disponibili, con provenienza, stati di assenza e un avviso esplicito per le osservazioni incerte escluse dal calcolo; il circuito usa geometria LMU identificata o segnala che non è disponibile un tracciato. Gara, Dati, Piano, Revisioni, Stint, Sosta, libreria e cronologia condividono il tavolo v5 in grigi Orbit, senza animare tutta l'interfaccia all'ingresso. Restano ricerca e selezione di fino a quattro sessioni, originali intatti, correzioni, calcolo Go, annullamento, copertura parziale, modifica e accettazione separata dal salvataggio. Mancano verifica nativa e visiva completa con DuckDB reali e accettazione di Isaac; HTML e test non chiudono T22. Restano anche preparazione endurance con memoria limitata (#1375), calibrazione indipendente e verifica temporale nativa. Live e ricerca OSS/Monte Carlo rimangono rinviati. Ramo isolato, senza integrazione o pubblicazione. Il lavoro locale #1395 applica già finestre temporali esatte per pilota in SolverV2; il tavolo #1396 ora le modifica in minuti di gara, le salva e le invia al calcolo.
- etiqueta: Feature
- etiqueta.pt: Feature
- etiqueta.it: Feature
...
- cuerpo: El recálculo de estrategia durante la carrera queda aplazado hasta completar y validar el editor de telemetría registrada. Después estudiaremos proyectos OSS y métodos de incertidumbre, incluido Monte Carlo, antes de decidir la arquitectura live. El SDD exige aceptación del registrado, con gates visuales, empíricos y nativos, antes de iniciar una investigación comparativa con licencias, experimentos y recomendación; no autoriza todavía la implementación live.
- cuerpo.en: In-race strategy recalculation is deferred until the recorded telemetry editor is complete and validated. We will then study OSS projects and uncertainty methods, including Monte Carlo, before deciding the live architecture. The SDD requires recorded-editor acceptance, including visual, empirical and native gates, before comparative research covering licenses, experiments and recommendations; it does not yet authorize live implementation.
- cuerpo.pt: O recálculo de estratégia durante a corrida fica adiado até concluir e validar o editor de telemetria registada. Depois estudaremos projetos OSS e métodos de incerteza, incluindo Monte Carlo, antes de decidir a arquitetura live. O SDD exige aceitação do editor registado, incluindo critérios visuais, empíricos e nativos, antes da pesquisa comparativa sobre licenças, experiências e recomendação; ainda não autoriza implementação live.
- cuerpo.it: Il ricalcolo della strategia in gara è rinviato fino al completamento e alla validazione dell'editor della telemetria registrata. Studieremo poi progetti OSS e metodi per l'incertezza, incluso Monte Carlo, prima di decidere l'architettura live. Il SDD richiede accettazione dell’editor registrato, inclusi criteri visivi, empirici e nativi, prima della ricerca comparativa su licenze, esperimenti e raccomandazioni; non autorizza ancora l’implementazione live.
- etiqueta: Plan
- etiqueta.pt: Plano
- etiqueta.it: Piano
...
- cuerpo: Eficiencia ofrece los estilos Signature y Broadcast para Standings, con transparencia suave, identidad Vantare, tipografía unificada y selección neutra. Cabecera y pie fino permiten elegir información de sesión; las diagonales reflejan la bandera sin animación cuando existe una señal vigente. La fuente REST LMU aporta temperaturas con calidad y caducidad y una bandera fail-closed; su equivalencia de códigos exige comprobación en sesión activa. Los perfiles conservan la selección y el marco adapta columnas, filas y pie en Studio, Desktop y OBS. La entrega conjunta quedó validada físicamente e integrada en Nightly mediante PR #1107; la equivalencia de bandera y temperatura sigue pendiente de una sesión LMU activa. La recuperación de los ajustes locales de Delta, Relative, Standings, Pedals y Fuel Strategy ha pasado las pruebas funcionales y los controles de calidad; Isaac ha autorizado su integración inicial en Nightly y la revisión visual continúa en Workshop, sin certificar todavía LMU activo. Las etiquetas de Eficiencia y el tipo de sesión comparten el idioma de la app en español, inglés, portugués e italiano. La preferencia se conserva y se propaga a las ventanas por eventos; los catálogos no se recalculan con la telemetría. Workshop autónomo comparte el idioma del navegador en su propio origen. Esta ampliación está integrada en Nightly mediante PR #1317; la comprobación física de Windows/OBS sigue pendiente. Relative incorpora deslizamientos suaves, fundidos breves y una señal de color muy tenue al cruzarse coches, manteniendo al jugador como referencia y las cifras estables. En carrera, Relative muestra una etiqueta discreta de vueltas por delante o por detrás del jugador a partir de la señal canónica del simulador; misma vuelta y datos no fiables no generan marca. Isaac ha confirmado la revisión de Relative en Workshop. Standings Eficiencia incorpora desplazamiento continuo de filas, avisos contenidos de posición y mejor vuelta, PIT unido a su piloto y transiciones de presencia. Horizontal Standings mueve las tarjetas al cambiar de orden, con fundidos breves y una señal tenue de ganancia o pérdida; cifras y bloques de sesión permanecen quietos. Workshop reúne escenas de las tres animaciones. Isaac autoriza la integración conjunta inicial de #1306, #1323 y #1333 en Nightly mediante ISA-1334; la revisión visual de Horizontal Standings y la prueba física Windows/OBS siguen pendientes. ISA-1328 ya está integrada en Nightly y añade un aviso de vuelta rápida con cronómetro morado, piloto y tiempo, ancho y alto editables. Avisa de la mejor vuelta personal de la sesión y de la clase propia, con prioridad a clase si ambas coinciden, sin alcance global. Incluye entrada, salida y repetición animadas; caduca a los seis segundos con duración configurable y Workshop reproduce el ciclo también en Studio. Comparte renderer entre Studio, Desktop, OBS y Workshop. Isaac ha aceptado el diseño y autorizado su incorporación inicial a Nightly mediante PR #1330; la validación física con LMU/Windows/OBS sigue pendiente en el canal. La revisión de datos de ISA-1347 concreta el contrato de los cinco widgets aceptados: referencias reales de Delta, ausencia por canal en Pedals, clasificación y referencias de clase en Standings, proximidad circular y vueltas relativas en Relative, y vuelta actual del jugador con carrusel configurable en Horizontal Standings. SOF queda fuera por ahora. Humedad REST y lluvia nativa conservan calidad y caducidad; el viento espera una unidad demostrada. La entrega sigue como candidata de revisión, con pruebas de regresión y validación física LMU/Windows/Desktop/OBS pendientes de cierre antes de certificar salida pública. Pedals Telemetry incorpora como candidato un selector persistente de 31 interpretaciones de volantes LMU Hypercar, LMGT3, LMP2 y LMP3, además del genérico. Comparte apariencia entre Studio, Desktop, OBS y Workshop; el catálogo está contrastado a 24/09/2026. La fidelidad visual por modelo y la dirección física siguen pendientes de aceptación y validación. El widget con volantes se llama Pedales avanzados. El compacto deja de ofrecerse en Studio y Workshop; los perfiles existentes mantienen sus IDs, ajustes y renderizado como Pedales antiguos, sin conversión destructiva. El candidato incorpora correcciones de datos: distingue cero de ausencia, limita pedales a 0–1 y vacía valores durante la conexión o parada; mantiene pendiente la dirección física y no aporta posición V2. Racing Flags Eficiencia presenta el nombre de la bandera con letra ampliada y centrada, negra por defecto y blanca automáticamente sobre la bandera negra; la verificación física LMU/Windows/OBS continúa pendiente. Car Damage Numbers Eficiencia muestra cuatro filas separadas para Aero, Body, Susp y neumáticos; el daño y el desgaste de la rueda más gastada proceden del frame Overlay V2 con calidad explícita de LMU. Isaac autoriza la integración inicial por PR #1386; sigue pendiente contrastar la escala de desgaste con LMU en marcha.
- cuerpo.en: Efficiency offers Signature and Broadcast Standings styles, with subtle transparency, Vantare identity, unified typography and neutral selection. The header and thin footer offer selectable session information; diagonal accents reflect the flag without animation when a current signal exists. The LMU REST source supplies temperatures with quality and expiry plus a fail-closed flag; its code mapping requires an active-session check. Profiles retain selections and the frame fits columns, rows and footer across Studio, Desktop and OBS. The joint delivery was physically validated and integrated into Nightly through PR #1107; flag and temperature equivalence still awaits an active LMU session. Recovery of local Delta, Relative, Standings, Pedals and Fuel Strategy refinements has passed functional tests and quality checks; Isaac has authorized initial Nightly integration and visual review continues in Workshop, without active LMU certification yet. Efficiency labels and session type share the app language in Spanish, English, Portuguese and Italian. The preference persists and reaches windows through events; catalogues are not recalculated with telemetry. Standalone Workshop shares the browser language preference within its own origin. This extension is integrated into Nightly through PR #1317; physical Windows/OBS validation remains pending. Relative includes gentle row movement, brief fades and a very faint colour cue when cars cross, keeping the player as a stable reference and numbers steady. In races, Relative shows a discreet lap-difference badge relative to the player using the simulator’s canonical signal; same-lap and unreliable values produce no badge. Isaac has confirmed the Relative review in Workshop. Efficiency Standings adds continuous row movement, restrained position and best-lap notices, PIT attached to its driver, and presence transitions. Horizontal Standings moves cards when their order changes, with brief fades and a faint gain or loss cue; numbers and session blocks remain still. Workshop brings together scenes for all three animations. Isaac authorizes the initial joint integration of #1306, #1323 and #1333 into Nightly through ISA-1334; visual review of Horizontal Standings and physical Windows/OBS checks remain pending. ISA-1328 is now integrated into Nightly and adds of a fastest-lap alert with a purple stopwatch, driver and time, plus editable width and height. It announces current-session personal bests and player-class records, with class priority when both coincide and no overall-field scope. Entry, exit and repeated alerts are animated; it expires after six seconds with configurable duration, and Workshop can play the cycle in Studio. Studio, Desktop, OBS and Workshop share its renderer. Isaac has accepted the design and authorized initial Nightly integration through PR #1330; physical LMU/Windows/OBS validation remains pending on that channel. The ISA-1347 data review defines the contract for the five accepted widgets: actual Delta references, per-channel absence in Pedals, classification and class references in Standings, circular proximity and relative laps in Relative, and the player’s current lap with a configurable carousel in Horizontal Standings. SOF is excluded for now. REST wetness and native rain retain quality and expiry; wind awaits a demonstrated unit. This remains a review candidate, with regression checks and physical LMU/Windows/Desktop/OBS validation requiring closure before public-release certification. Pedals Telemetry adds a candidate persistent selector for 31 interpretations of LMU Hypercar, LMGT3, LMP2 and LMP3 steering wheels plus the generic wheel. Studio, Desktop, OBS and Workshop share the appearance; the vehicle catalogue was checked on 2026-09-24. Per-model visual fidelity and physical steering remain subject to acceptance and validation. The widget with steering wheels is named Advanced pedals. The compact widget is no longer offered in Studio or Workshop; existing profiles retain their IDs, settings and rendering as Legacy pedals, without destructive conversion. The candidate incorporates data corrections: it distinguishes zero from missing data, bounds pedals to 0–1 and clears values while connecting or stopping; physical steering remains pending and V2 position is still unavailable. Efficiency Racing Flags displays the flag name in larger centered lettering, black by default and automatically white on a black flag; physical LMU/Windows/OBS verification remains pending. Efficiency Car Damage Numbers displays four separate rows for Aero, Body, Susp and tyres; damage and the most worn tyre come from the LMU Overlay V2 frame with explicit signal quality. Isaac authorizes initial integration through PR #1386; physical LMU verification of the wear scale remains pending.
- cuerpo.pt: Eficiência oferece os estilos Signature e Broadcast para Standings, com transparência suave, identidade Vantare, tipografia unificada e seleção neutra. Cabeçalho e rodapé fino permitem escolher informação da sessão; as diagonais refletem a bandeira sem animação quando existe um sinal atual. A fonte REST LMU fornece temperaturas com qualidade e expiração e uma bandeira fail-closed; o mapeamento dos códigos exige verificação numa sessão ativa. Os perfis preservam a seleção e o quadro adapta colunas, filas e rodapé no Studio, Desktop e OBS. A entrega conjunta foi validada fisicamente e integrada na Nightly através do PR #1107; a equivalência da bandeira e da temperatura continua pendente de uma sessão LMU ativa. A recuperação dos ajustes locais de Delta, Relative, Standings, Pedals e Fuel Strategy passou nos testes funcionais e controlos de qualidade; Isaac autorizou a integração inicial na Nightly e a revisão visual continua no Workshop, ainda sem certificar LMU ativo. As etiquetas de Eficiência e o tipo de sessão partilham o idioma da app em espanhol, inglês, português e italiano. A preferência é guardada e propagada às janelas por eventos; os catálogos não são recalculados com a telemetria. O Workshop autónomo partilha a preferência de idioma do navegador na sua própria origem. Esta ampliação está integrada na Nightly através do PR #1317; a validação física em Windows/OBS continua pendente. Relative inclui movimentos suaves, transições breves de opacidade e um sinal de cor muito discreto quando os carros se cruzam, mantendo o jogador como referência e os números estáveis. Em corrida, Relative mostra uma etiqueta discreta de diferença de voltas em relação ao jogador usando o sinal canónico do simulador; mesma volta e dados sem qualidade não geram marca. Isaac confirmou a revisão de Relative no Workshop. Standings Eficiência acrescenta movimento contínuo das filas, avisos discretos de posição e melhor volta, PIT ligado ao piloto e transições de presença. Horizontal Standings move os cartões quando a ordem muda, com breves transições de opacidade e um sinal discreto de ganho ou perda; números e blocos da sessão ficam imóveis. O Workshop reúne cenas das três animações. Isaac autoriza a integração inicial conjunta de #1306, #1323 e #1333 na Nightly por ISA-1334; a revisão visual de Horizontal Standings e a verificação física Windows/OBS continuam pendentes. ISA-1328 já está integrada na Nightly e adiciona um aviso de volta mais rápida com cronómetro roxo, piloto e tempo, largura e altura editáveis. Avisa da melhor volta pessoal da sessão e do recorde da classe do jogador, com prioridade à classe quando coincidem, sem âmbito global. Entrada, saída e repetição são animadas; desaparece após seis segundos com duração configurável e o Workshop reproduz o ciclo também no Studio. Studio, Desktop, OBS e Workshop partilham o renderer. Isaac aceitou o design e autorizou a integração inicial na Nightly através do PR #1330; a validação física com LMU/Windows/OBS continua pendente no canal. A revisão de dados ISA-1347 define o contrato dos cinco widgets aceites: referências reais de Delta, ausência por canal em Pedals, classificação e referências de classe em Standings, proximidade circular e voltas relativas em Relative, e volta atual do jogador com carrossel configurável em Horizontal Standings. SOF fica excluído por enquanto. Humidade REST e chuva nativa conservam qualidade e validade; o vento aguarda uma unidade demonstrada. A entrega continua candidata a revisão, com testes de regressão e validação física LMU/Windows/Desktop/OBS por concluir antes de certificar o lançamento público. Pedals Telemetry incorpora como candidato um seletor persistente de 31 interpretações de volantes LMU Hypercar, LMGT3, LMP2 e LMP3, além do genérico. Studio, Desktop, OBS e Workshop partilham a aparência; o catálogo foi confirmado em 24/09/2026. A fidelidade visual de cada modelo e a direção física aguardam aceitação e validação. O widget com volantes passa a chamar-se Pedais avançados. O compacto deixa de ser oferecido no Studio e Workshop; os perfis existentes mantêm IDs, ajustes e apresentação como Pedais antigos, sem conversão destrutiva. O candidato incorpora correções de dados: distingue zero de ausência, limita os pedais a 0–1 e limpa valores durante a ligação ou paragem; a direção física continua pendente e a posição V2 permanece indisponível. Racing Flags Eficiência apresenta o nome da bandeira com letras maiores e centradas, pretas por predefinição e automaticamente brancas sobre a bandeira preta; a verificação física LMU/Windows/OBS continua pendente. Car Damage Numbers Eficiência apresenta quatro linhas separadas para Aero, Body, Susp e pneus; os danos e o pneu mais gasto vêm do frame Overlay V2 de LMU com qualidade explícita. Isaac autoriza a integração inicial através do PR #1386; ainda falta verificar a escala de desgaste com LMU em funcionamento.
- cuerpo.it: Efficienza offre gli stili Signature e Broadcast per Standings, con trasparenza lieve, identità Vantare, tipografia unificata e selezione neutra. Intestazione e piè sottile permettono di scegliere informazioni di sessione; le diagonali riflettono la bandiera senza animazione quando esiste un segnale attuale. La fonte REST LMU fornisce temperature con qualità e scadenza e una bandiera fail-closed; la mappatura dei codici richiede una verifica in sessione attiva. I profili conservano la selezione e il riquadro adatta colonne, righe e piè in Studio, Desktop e OBS. La consegna congiunta è stata verificata fisicamente e integrata in Nightly tramite la PR #1107; l'equivalenza di bandiera e temperatura resta in attesa di una sessione LMU attiva. Il recupero degli aggiornamenti locali di Delta, Relative, Standings, Pedals e Fuel Strategy ha superato i test funzionali e i controlli di qualità; Isaac ha autorizzato l’integrazione iniziale in Nightly e la revisione visiva continua in Workshop, senza certificare ancora LMU attivo. Le etichette di Efficienza e il tipo di sessione condividono la lingua dell’app in spagnolo, inglese, portoghese e italiano. La preferenza viene conservata e propagata alle finestre tramite eventi; i cataloghi non vengono ricalcolati con la telemetria. Workshop autonomo condivide la preferenza di lingua del browser nella propria origine. Questa estensione è integrata in Nightly tramite la PR #1317; la verifica fisica di Windows/OBS resta da completare. Relative include movimenti delicati, brevi dissolvenze e un segnale di colore molto tenue quando le auto si incrociano, mantenendo il giocatore come riferimento e le cifre stabili. In gara, Relative mostra un’etichetta discreta della differenza di giri rispetto al giocatore usando il segnale canonico del simulatore; stesso giro e dati non affidabili non generano alcun indicatore. Isaac ha confermato la revisione di Relative in Workshop. Standings Efficienza aggiunge movimento continuo delle righe, avvisi discreti di posizione e miglior giro, PIT legato al pilota e transizioni di presenza. Horizontal Standings sposta le schede quando cambia l’ordine, con brevi dissolvenze e un segnale tenue di guadagno o perdita; cifre e blocchi della sessione restano fermi. Workshop riunisce le scene delle tre animazioni. Isaac autorizza l’integrazione iniziale congiunta di #1306, #1323 e #1333 in Nightly tramite ISA-1334; la revisione visiva di Horizontal Standings e la verifica fisica Windows/OBS restano in sospeso. ISA-1328 è ora integrata in Nightly e aggiunge un avviso di giro veloce con cronometro viola, pilota e tempo, larghezza e altezza modificabili. Segnala il miglior giro personale della sessione e il record della propria classe, con priorità alla classe quando coincidono, senza ambito globale. Entrata, uscita e ripetizione sono animate; scompare dopo sei secondi con durata configurabile e Workshop riproduce il ciclo anche in Studio. Studio, Desktop, OBS e Workshop condividono il renderer. Isaac ha accettato il design e autorizzato l’integrazione iniziale in Nightly tramite la PR #1330; la verifica fisica con LMU/Windows/OBS resta da completare nel canale. La revisione dati ISA-1347 definisce il contratto dei cinque widget accettati: riferimenti reali di Delta, assenza per canale in Pedals, classifica e riferimenti di classe in Standings, prossimità circolare e giri relativi in Relative, e giro attuale del giocatore con carosello configurabile in Horizontal Standings. SOF resta escluso per ora. Umidità REST e pioggia nativa conservano qualità e scadenza; il vento attende un’unità dimostrata. La consegna resta candidata alla revisione, con test di regressione e verifica fisica LMU/Windows/Desktop/OBS da completare prima di certificare il lancio pubblico. Pedals Telemetry aggiunge come candidato un selettore persistente di 31 interpretazioni di volanti LMU Hypercar, LMGT3, LMP2 e LMP3, oltre al generico. Studio, Desktop, OBS e Workshop condividono l’aspetto; il catalogo è stato verificato il 24/09/2026. La fedeltà visiva di ogni modello e lo sterzo fisico restano da accettare e validare. Il widget con volanti si chiama Pedali avanzati. Il compatto non viene più proposto in Studio o Workshop; i profili esistenti conservano ID, impostazioni e visualizzazione come Pedali precedenti, senza conversione distruttiva. Il candidato incorpora correzioni dei dati: distingue lo zero dai dati assenti, limita i pedali a 0–1 e cancella i valori durante connessione o arresto; lo sterzo fisico resta da verificare e la posizione V2 non è disponibile. Racing Flags Efficienza mostra il nome della bandiera con caratteri più grandi e centrati, neri per impostazione predefinita e automaticamente bianchi sulla bandiera nera; la verifica fisica LMU/Windows/OBS resta in sospeso. Car Damage Numbers Efficienza mostra quattro righe separate per Aero, Body, Susp e pneumatici; i danni e lo pneumatico più usurato provengono dal frame Overlay V2 di LMU con qualità esplicita. Isaac autorizza la prima integrazione tramite la PR #1386; resta da verificare la scala di usura con LMU in esecuzione.
- etiqueta: ISA-1083
...
- cuerpo: Studio, Desktop y OBS aplican la misma política nativa por widget desde la licencia verificada, sin autoridad de licencia en renderizadores. Standings y Pedals siguen disponibles en Free y con cualquier pago; Delta y los demás premium exigen Overlays; engineer-radio exige Engineer y Overlays no lo concede. Los perfiles conservan los widgets bloqueados al importar, mover o eliminar y recuperan sus ajustes al volver los derechos. Crystal y Efficiency muestran la marca Vantare obligatoria en Free, también con cabecera o pie ocultos o ajustes importados, y la ocultan por defecto con pago con preferencia explícita para activarla. La entrega conjunta quedó validada e integrada en Nightly mediante PR #1107. Sin testers/master ni release.
- cuerpo.en: Studio, Desktop and OBS enforce the same native per-widget policy from the verified license, with no licensing authority inside renderers. Standings and Pedals stay available on Free and any paid plan; Delta and other premium widgets require Overlays; engineer-radio requires Engineer and Overlays never grants it. Profiles keep blocked widgets across import, move or delete and restore their settings when rights return. Crystal and Efficiency show the mandatory Vantare brand on Free, even with hidden header or footer or imported settings, and hide it by default on paid plans with an explicit opt-in. The joint delivery was validated and integrated into Nightly through PR #1107. No testers/master or release.
- cuerpo.pt: Studio, Desktop e OBS aplicam a mesma política nativa por widget a partir da licença verificada, sem autoridade de licença nos renderers. Standings e Pedals continuam disponíveis no Free e em qualquer plano pago; Delta e os restantes premium exigem Overlays; engineer-radio exige Engineer e Overlays nunca o concede. Os perfis conservam os widgets bloqueados ao importar, mover ou eliminar e recuperam os ajustes quando os direitos voltam. Crystal e Efficiency mostram a marca Vantare obrigatória no Free, mesmo com cabeçalho ou rodapé ocultos ou ajustes importados, e ocultam-na por defeito nos pagos com ativação explícita. A entrega conjunta foi validada e integrada na Nightly através do PR #1107. Sem testers/master nem release.
- cuerpo.it: Studio, Desktop e OBS applicano la stessa policy nativa per widget dalla licenza verificata, senza autorità di licenza nei renderer. Standings e Pedals restano disponibili su Free e qualsiasi piano a pagamento; Delta e gli altri premium richiedono Overlays; engineer-radio richiede Engineer e Overlays non lo concede mai. I profili conservano i widget bloccati tra importazione, spostamento o eliminazione e ne ripristinano le impostazioni al ritorno dei diritti. Crystal ed Efficiency mostrano il marchio Vantare obbligatorio su Free, anche con intestazione o piè nascosti o impostazioni importate, e lo nascondono di default sui piani a pagamento con attivazione esplicita. La consegna congiunta è stata verificata e integrata in Nightly tramite la PR #1107. Nessun testers/master né release.
- etiqueta: ISA-1105
