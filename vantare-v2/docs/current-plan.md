...
- Clock UTC inyectable y tests sin `time.Sleep`. TDD cubre epoch/identity, source/participantes, ausencia/reaparición, reconexión, resets sesión/run, headers por hecho, driver/team, lap/pit, duplicados/gaps, orden/secuencia/ownership, overflow, matriz `Apply`/`SetConnected`/`EndSession` contra cierre/backpressure, lectura concurrente durante commit bloqueado, determinismo, fuzz con oráculo independiente y benchmark con 64 vehículos.
- Guía de contrato y verificación: `docs/telemetry-core/session-coordinator.md`. Evidencia fresca de la corrección: focal core/catalog/LMU x20, Telemetry Core completo, guard ADR 0004, vet focal, suite global Go, race coordinator x10 con GCC UCRT64, fuzz con oráculo 10 s (555.959 ejecuciones), build frontend y `git diff --check` PASS. Benchmark aislado con 64 vehículos: 16,8–20,7 µs/op, 38.144 B/op y 9 allocs/op. Frontend focal `useCanvasInteraction` 24/24 PASS y suite completa 280 archivos, 1851/1851 PASS; Vitest todavía imprime al teardown el `AbortError` no fatal conocido de happy-dom después del resultado verde y devuelve código 0. El vet amplio conserva tres warnings `unsafe.Pointer` Win32 heredados fuera del diff; no hay archivos frontend en el cambio.
