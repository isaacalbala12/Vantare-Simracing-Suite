...
\if :{?dblink_password}
\else
\set dblink_password postgres
\endif

create extension if not exists pgtap with schema extensions;
create extension if not exists dblink with schema extensions;

\if :{?dblink_unrestricted}
create or replace function pg_temp.webhook_test_dblink_connect(connection_name text, connection_string text)
returns text language sql as $$
  select extensions.vantare_test_dblink_connect(connection_name, connection_string)
$$;
\else
create or replace function pg_temp.webhook_test_dblink_connect(connection_name text, connection_string text)
returns text language sql as $$
  select extensions.dblink_connect(connection_name, connection_string)
$$;
\endif
...
select is(
  pg_temp.webhook_test_dblink_connect(
    'billing_worker_one',
    format(
      'host=127.0.0.1 dbname=%I user=postgres password=%s',
      current_database(),
      :'dblink_password'
    )
  ),
