...
- cuerpo: La frontera de identidad crea o recupera un UUID interno desde una sesión Clerk validada por Supabase TPA y firma la credencial de licencia con ese UUID. El frontend ya monta el acceso oficial, pide un token vigente y cierra la sesión sin persistir el JWT; la prueba Wails real, Billing, Testing Center y los demás servicios ligados a Supabase Auth siguen pendientes.
- cuerpo.en: The identity boundary creates or retrieves an internal UUID from a Clerk session validated by Supabase TPA and signs the licence credential with that UUID. The frontend now mounts the official sign-in, requests a current token and signs out without persisting the JWT; real Wails proof, Billing, Testing Center and the other Supabase Auth-bound services remain pending.
- cuerpo.pt: A fronteira de identidade cria ou recupera um UUID interno a partir de uma sessão Clerk validada pelo Supabase TPA e assina a credencial de licença com esse UUID. O frontend já monta o login oficial, pede um token atual e termina a sessão sem persistir o JWT; a prova Wails real, Billing, Testing Center e os restantes serviços ligados ao Supabase Auth continuam pendentes.
- cuerpo.it: Il confine di identità crea o recupera un UUID interno da una sessione Clerk validata da Supabase TPA e firma la credenziale di licenza con tale UUID. Il frontend ora monta l'accesso ufficiale, richiede un token aggiornato ed esce senza persistere il JWT; la prova Wails reale, Billing, Testing Center e gli altri servizi legati a Supabase Auth restano in sospeso.
- etiqueta: Feature
