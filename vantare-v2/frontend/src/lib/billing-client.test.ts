...
describe('billing-client', () => {
  ...
  it('createBillingCheckout sends a local idempotency UUID with Bearer token', async () => {
  ...
  it('reuses the product attempt id for a local retry', async () => {
    ...
    const first = JSON.parse(String(fetchSpy.mock.calls[0][1]?.body)) as Record<string, unknown>;
    const second = JSON.parse(String(fetchSpy.mock.calls[1][1]?.body)) as Record<string, unknown>;
    expect(first.productKey).toBe('pro_monthly');
  ...
  it('maps billing_customer_not_found from portal 404', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
      new Response(JSON.stringify({ error: 'billing_customer_not_found' }), { status: 404 }),
    );
