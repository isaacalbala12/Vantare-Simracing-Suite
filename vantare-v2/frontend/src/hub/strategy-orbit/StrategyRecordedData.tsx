...
export function StrategyRecordedData({ controller, sessions, sessionLabels = {}, busy, onSources, onPendingChange, t }: {
  ...
  function clearForm() { setFamilyForm(null); setForm(null); setFormDirty(false); setFormError(false); onPendingChange(false); }
  ...
  return <section className="strategy-recorded-data" aria-labelledby="recorded-data-title">
    ...
    <div className="strategy-recorded-data__grid">
      <section className="strategy-recorded-data__observations">
        ...
        <div className="strategy-recorded-laps__views"><h3>{t("strategy.data.observations")}</h3><Button size="sm" variant="ghost" disabled={locked || formDirty} onClick={() => { clearForm(); setView(view === "laps" ? "samples" : "laps"); }}>{t(view === "laps" ? "strategy.laps.advanced" : "strategy.laps.back")}</Button></div>
        ...
        {!editor ? <div className="strategy-recorded-data__empty"><Icon name="i-telemetria" size={44} /><strong>{t("strategy.data.chooseSource")}</strong><p>{t("strategy.data.chooseSourceHint")}</p></div> : view === "laps" ? <RecordedLapList page={editor.lapPage} family={family} proposals={editor.familyUses ?? []} selected={familyForm?.row} locked={locked || formDirty || Boolean(editor.request)} t={t}
          ...
          onSelect={(row, proposal) => { clearForm(); setFamilyForm({ row, family, choice: proposal ? proposal.included ? "include" : "exclude" : "automatic", reason: proposal?.reason ?? "" }); }} /> : <>
      ...
      <aside className="strategy-recorded-data__detail"><h3>{t("strategy.data.review")}</h3><p className="strategy-recorded-data__muted">{t("strategy.data.reviewHint")}</p>
        {familyForm ? <RecordedLapDetail form={familyForm} dirty={formDirty} locked={locked || Boolean(editor?.request)} t={t} onChange={next => { setFamilyForm(next); setFormDirty(true); onPendingChange(true); }} onApply={apply} onCancel={clearForm} /> : form ? <form onSubmit={event => { event.preventDefault(); apply(); }}>
        ...
        </form> : <div className="strategy-recorded-data__empty"><Icon name="i-ajustes" size={36} /><p>{t(view === "laps" ? "strategy.laps.chooseLap" : "strategy.data.chooseSample")}</p></div>}
        {editor ? <div className="strategy-recorded-data__revision">
          <strong>{t("strategy.data.pendingCount")} {editor.corrections.length + (editor.familyUses?.length ?? 0)}</strong>
