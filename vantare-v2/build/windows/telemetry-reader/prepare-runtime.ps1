...
$bin = [System.IO.Path]::GetFullPath((Join-Path $repo $BinDir)).TrimEnd("\")
$repoPrefix = $repo + [System.IO.Path]::DirectorySeparatorChar
if (-not $bin.StartsWith($repoPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "BinDir must be contained by RepoRoot. RepoRoot=$repo BinDir=$bin"
}
