...
- cuerpo: Strategy permite guardar una copia DuckDB verificada en una carpeta elegida sin modificar el original. También permite seleccionar un archivo fuera de las carpetas descubiertas y exige la misma fuente y revisión al aplicarlo a un borrador guardado. Pendiente registrar la ubicación bajo Analysis y ofrecer recuperación desde esa copia sólo si falta el original, además de QA Wails tras reinicio. La copia temporal de lectura no es una copia conservada.
- cuerpo.en: Strategy can save a verified DuckDB copy in a chosen folder without changing the original. Users can select a file outside discovered folders, and a saved draft requires the same source and revision before applying it. Analysis still needs to retain the copy location and offer recovery only when the original is missing, followed by Wails QA after restart. The temporary reading copy is not retained.
- cuerpo.pt: Strategy permite guardar uma cópia DuckDB verificada numa pasta escolhida sem alterar o original. Também permite escolher um ficheiro fora das pastas descobertas e exige a mesma fonte e revisão antes de o aplicar a um rascunho guardado. Falta registar a localização em Analysis e oferecer recuperação só quando o original faltar, além de QA Wails após reinício. A cópia temporária de leitura não é conservada.
- cuerpo.it: Strategy permette di salvare una copia DuckDB verificata in una cartella scelta senza modificare l'originale. Si può anche scegliere un file fuori dalle cartelle scoperte; un documento salvato richiede la stessa fonte e revisione prima di applicarlo. Restano da registrare la posizione in Analysis e offrire recupero solo se manca l'originale, oltre al QA Wails dopo il riavvio. La copia temporanea di lettura non viene conservata.
- etiqueta: Pendiente
- etiqueta.en: Planned
