...
El seguimiento está en [PR draft #1354](https://github.com/isaacalbala12/Vantare-Simracing-Suite/pull/1354), rama `vantareapp/isa-1347-nightly-parser-perf` desde el SHA integrado. Los heads `1be6fdf9`, `f7fac47f` y `6de1dd3c` pasaron promoción/ratchet; el gate frontend midió compacto en 1,656, 1,624 y 1,530 ms respectivamente, frente a 1,5 ms. `bae7c02e` reutiliza el buffer UTF-8 con límite exacto de 72 KiB; `264ceb86` normaliza directamente filas compactas comunes sin saltar validación; `c953c82a` evita repetir checks de prototipo tras parseo JSON, conservando el check del request externo. Suite focal del último código 42/42, legacy 0,688 ms y compacto 0,812 ms; typecheck, lint y build PASS. La suite completa anterior pasó 481 archivos, 4.036 pruebas (2 omitidas); CI del nuevo head pendiente.

La issue [#1347](https://github.com/isaacalbala12/Vantare-Simracing-Suite/issues/1347) sigue abierta en `state:nightly`. La prueba física con LMU activo en Windows y los consumidores Desktop/OBS permanece pendiente; no hay promoción a testers/master ni release. Siguiente paso: completar los gates de #1354 y continuar esa verificación en el build integrado.
