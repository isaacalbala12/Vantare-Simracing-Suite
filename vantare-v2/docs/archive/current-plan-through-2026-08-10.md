# Archivo histórico de `current-plan`
...
- El plan que se declaró ejecutable en ese momento fue
  `docs/overlays-studio/overlay-studio-v1-commercial-launch-plan.md`. Billing y
  venta seguían NO-GO hasta cumplir sus gates y recibir aprobación explícita.
- ISA-315 solo documentaba y coordinaba. No autorizaba promoción, migración,
  dinero, publicación, merge ni release.
- Isaac autorizó por separado el merge de PR #198 a `nightly` una vez resuelto
  ISA-311. Esa autorización no alcanzaba `testers`, `master`, venta ni release.
...
- La corrección inyectó exclusivamente en ese test un reloj lógico fijo y un
  ...
  No cambió `DefaultCommitBudget`, su validación máxima, los contextos del
  runtime ni la clasificación de fallos de producción.
- Evidencia registrada entonces: baseline previo 10/10; el primer ajuste de
  reloj pasó 20/20, pero una repetición posterior encontró 1/20 cierres por el
  timeout que aún llegaba al writer. La solución completa pasó soak 20/20,
  regresiones de timeout del coordinator 20/20, build frontend y
  `go test ./... -count=1`.
- PR #200 se promovió por rebase a `nightly@54f267b` tras los runs verdes
  `31416018600`, `31416779711` y `31435630710`, todos sin rerun. Linear reflejó
...
Microcorte 2 completado: Panel 'Próximas carreras' implementado e integrado en el Hub. Limpieza de componentes obsoletos realizada.
...
Nota CIERRE-ALINEACION-REFACTOR (2026-08-10, documentado en local):
- Estado final: la documentacion de conservacion quedo integrada en
  `origin/nightly@08fcfc1` via PR #189 (`docs(conservacion)` + `docs(workflow)`,
  squash, 3 checks PASS). `refactor` local alineado con `nightly` en ese corte.
- ADR de telemetria renumerados a `0090`/`0091` (sufijo `-legacy`) en la rama
  `chore/conservacion-untracked-2026-08-10` (commits `ac53ab4`, `77ecdd4`);
  la version integrada en Nightly usa esos numeros.
- NO integradas (pendientes de decision, solo en la rama de conservacion):
  2 funciones Supabase del Testing Center (`testing-center-codex-dossier.ts`,
  `testing-center-rejection.ts`) por import roto de
  `testing-center-projection-sanitization.ts` que no existe en el repo.
