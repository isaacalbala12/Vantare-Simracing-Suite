...
function sample(snapshot: TelemetrySnapshot): InputTelemetrySample { return { capturedAt: snapshot.capturedAt, throttle: readNormalizedInput(snapshot.player.throttle) ?? 0, brake: readNormalizedInput(snapshot.player.brake) ?? 0, clutch: readNormalizedInput(snapshot.player.clutch) ?? 0, speedKph: readNonNegativeNumber(snapshot.player.speedKph), rpm: readNonNegativeNumber(snapshot.player.rpm), gear: readFiniteNumber(snapshot.player.gear) }; }
...
/** Clears one widget history without disrupting other active widget instances. */
export function clearInputTelemetryHistory(widgetId: string): void { histories.delete(widgetId); }
