...
describe("RacingFlagsCrystal", () => { it("renders the isolated global flag card without an external sector summary", () => { const { container } = render(<RacingFlagsCrystal model={model} settings={{}} renderMode="harness" />); const root = container.querySelector('[data-widget-system="vantare-crystal"]') as HTMLElement; expect(root.querySelector(".vc-racing-flags-top")).toBeTruthy(); expect(root.querySelectorAll("[data-sector]")).toHaveLength(0); expect(root.textContent).not.toContain("S1"); }); });
