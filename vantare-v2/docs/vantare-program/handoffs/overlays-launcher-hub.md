...
### 2026-09-24 · ISA-1361 · anchura predeterminada de Vuelta rápida

Isaac solicita estirar moderadamente el widget en horizontal, conservando sus proporciones interiores. La propuesta cambia solo el tamaño inicial y la previsualización de Workshop de 480×104 a 528×104 (+10 % de anchura); las instancias guardadas, el mínimo 280×72, los datos y las animaciones conservan su comportamiento. El renderer flexible reparte el espacio adicional sin transformar geométricamente el cronómetro circular ni las fuentes. El mismo componente sirve a Studio, Desktop y OBS.

Workshop aislado confirma aviso personal y de clase a 528×104 y el modo compacto a 320 px sin recortes. Suite frontend completa con Node 22: 481 archivos y 4.036 pruebas aprobadas (2 omitidas); 72 focales, typecheck, lint, build, digest de roadmap y quality ratchet aprobados (NEW=0, MOVED=0, policy_changed=false). Node 26 alpha falla pruebas no relacionadas por su implementación de `localStorage`; no se considera entorno válido para la suite. El ajuste se sigue en [Asana](https://app.asana.com/1/1210926733859493/project/1218742976551956/task/1218762634127535) y la [issue técnica #1361](https://github.com/isaacalbala12/Vantare-Simracing-Suite/issues/1361). Pendiente revisión visual del usuario antes de integrar esta ampliación; no cambia la aceptación del diseño anterior ni certifica telemetría física LMU/Windows/OBS.
