...
## VAN-747 / GitHub #1325 — candidato Go 1.27.1 verificado en Windows (2026-09-23)

[Tarea Notion VAN-747](https://app.notion.com/p/3e3e51695c6581329168eec61e2dbb7e), [referencia técnica #1325](https://github.com/isaacalbala12/Vantare-Simracing-Suite/issues/1325). Rama aislada `vantareapp/isa-1325-go-127-toolchain`, base inicial Nightly `e6d7d2b5e58f55b82c0ed2f6a79667476d897086`, base actual incorporada `b725c402c2e77350f348cfc26dcc1b008c0ae2bd`.

El plan [Go 1.27.1](../../specs/go-127-toolchain/PLAN.md) recibió `REQUEST_CHANGES` del mismo subagente Astra que revisó el informe de arquitectura. Antes de implementar se incorporaron el pin oculto de Doctor, la recalibración explícita de baselines, los cuatro módulos de tidy, la comprobación del paso de roadmap en modo auditoría y la limitación de Docker/helper nativo. Dictamen posterior: GO para producir candidato. Wails permanece en beta.24 y el helper precompilado de telemetría no se reconstruye. Los Dockerfiles alternativos tienen fallos previos de punto de entrada y Garble antiguo; quedan fuera de este PR y no se presentan como builds probadas.

Go 1.27.1 se ha descargado y seleccionado localmente; cuatro `go mod tidy -diff` PASS, Doctor PASS, prueba focal de versión PASS, build frontend y cross build Windows PASS. Analizadores locales recompilados con Go 1.27.1. El mismo subagente Astra dio ACCEPT al diff `49e6d4c4`: cero P0/P1/P2 introducidos, cuatro baselines revisados y veinte `SA4023` heredados de la CLI administrativa identificados explícitamente. [Los dos gates obligatorios del PR #1327](../../analysis/isa-1325-go-127-verification.md) pasaron en ese commit de código: contrato de promoción/roadmap y gate Windows con tests Go/frontend y build Wails. Quality terminó `REVIEW_REQUIRED` solo por política modificada, NEW/MOVED/integridad cero; la revisión independiente quedó satisfecha sin falsear el estado del job. El host macOS no pasó `go test ./...` por fallos de launcher, diagnóstico y grabación/SQLite, que no se atribuyen sin prueba a esta migración. Las rutas Docker, los informes ocultos no subidos por CI Quality y los stubs de CLI quedaron como tareas pendientes separadas en Notion.

**Bloqueo histórico:** el commit `4bd4b43d` solo cambió documentación, pero dos ejecuciones del gate Windows en ese SHA fallaron en el mismo test por timeout de ocho segundos; el SHA documental `71fb6e01` reprodujo el fallo. Ese test había pasado en `49e6d4c4`. No hay base para atribuir causalmente la inestabilidad a Go 1.27.1. El ratchet de esos SHA repitió `REVIEW_REQUIRED` sin hallazgos nuevos ni movimientos.

**Reanudación 2026-09-23:** Isaac autorizó completar la migración, incluyendo ese test concreto. El candidato comprueba la existencia del archivo antes de iniciar el reproductor, conserva cancelación y proceso activo, y ajusta sus pruebas Windows. Gofmt, test local del paquete, compilación cruzada del ejecutable de pruebas, `GOOS=windows go vet ./...` y staticcheck focal PASS. En `3cca49f6`, [promoción/roadmap](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35802080443/job/106994475202) y [gate Windows](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35802080443/job/106994546114) **SUCCESS**: suite Go completa, frontend, gate visual y build Wails. [Quality](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35802080441/job/106994474355) conserva `REVIEW_REQUIRED` por política, con NEW/MOVED/RESOLVED=0 e integridad correcta. [Plan ampliado](../../specs/go-127-toolchain/PLAN.md) y [evidencia detallada](../../analysis/isa-1325-go-127-verification.md). Los checks obligatorios del HEAD final del PR #1327 y la base remota Nightly se verifican antes de solicitar integración. Nightly, release y runtime publicado siguen sin cambios.

**Base vigente y rendimiento 2026-09-23:** por petición de Isaac se incorporó `nightly@8b25d076` en `5fc8fff8` sin force-push; el digest generado del roadmap se regeneró y verificó. El [informe de rendimiento](../../analysis/isa-1325-go-127-performance.md) documenta un A/B de diez pares Windows sobre el mismo árbol: `WriteBatch` sintético hasta snapshot retenido, con demanda registrada pero sin entrega al frontend, empeora un 39,1 % en tiempo y 34,8 % en bytes/op con Go 1.27.1 predeterminado. El opt-out temporal `GOEXPERIMENT=nojsonv2` recupera un 30,5 % de tiempo y 25,6 % de bytes en una contraprueba Windows de diez pares; tests focales SUCCESS. Se aplicó en builds Windows canónicas, CI/release y scripts locales de medida/Orbit, con comprobación del binario. En `ac2cc763`, [promoción/roadmap y gate Windows](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35850775088) **SUCCESS**, incluida suite Go/frontend, Wails y metadato del EXE. [Quality](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35850775070) conserva `REVIEW_REQUIRED` por política, NEW=0, MOVED=0 en la pasada CI final; GitGuardian SUCCESS. Astra max revisó mitigación e informe: ACCEPT con reservas, sin P0–P2. El PR permanece draft, sin integración ni release; se comprueban los checks de todo HEAD posterior.

**Caché antes de integrar · 2026-09-23:** Isaac aceptó el orden propuesto por Astra max. `a4596943` guarda la parte estable de la política por revisión; los tests focales, `go vet` y su pasada con detector de carreras terminaron PASS. [A/B Windows de diez pares](https://github.com/isaacalbala12/Vantare-Simracing-Suite/actions/runs/35882663551): con JSON predeterminado, `WriteBatch` 463,2→385,0 µs (−16,9 %) y 406,7→297,5 KiB/op (−26,8 %); con `nojsonv2`, el cambio de tiempo 314,9→309,5 µs no es concluyente (`p=0,190`), aunque bajan las asignaciones. El opt-out permanece. [Datos y límites](../../analysis/isa-1325-go-127-performance.md). El merge `a43f9b9d` incorpora `nightly@b725c402`; dos conflictos de audio conservaron exactamente la versión de Nightly y el digest del roadmap pasó `--check`. PR #1327 vuelve a ser combinable, pero sus gates del nuevo HEAD están pendientes. Sin ensayo LMU/OBS, integración ni release; la retirada del opt-out sigue en [VAN-756](https://app.notion.com/p/3e4e51695c6581238b04f5ce3470e045).
