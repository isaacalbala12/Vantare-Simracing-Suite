...
export function FunctionalStudyControls({ query, update, modules, onModules }: {
  ...
  return <aside className="functional-study-controls" aria-label="Diseño del standings">
    ...
    <fieldset><legend>Dirección v2</legend>
      <div className="functional-study-segments">
        <button type="button" aria-pressed={!query.studyStyle} onClick={() => update({ ...query, studyStyle: undefined })}>V1</button>
        {FUNCTIONAL_STUDY_STYLES.filter((style) => !style.renderer).map((style) => <button type="button" key={style.id} aria-pressed={query.studyStyle === style.id} onClick={() => update({ ...query, designId: style.designId, studyStyle: style.id })}>{style.label}</button>)}
      </div>
      <div className="functional-study-segments functional-study-segments--stack">
        {FUNCTIONAL_STUDY_STYLES.filter((style) => style.renderer).map((style) => <button type="button" key={style.id} aria-pressed={query.studyStyle === style.id} onClick={() => update({ ...query, designId: style.designId, studyStyle: style.id })}>{style.label}</button>)}
      </div>
      <p className="functional-study-note">Muro y Escalera son vistas de estudio sobre los mismos datos; los colores de equipo son de demostración (la telemetría aún no los entrega).</p>
    </fieldset>
    ...
    <div className="functional-study-provenance"><span className="functional-study-dot" />Escenario de diseño<p>Datos de demostración. Las pieles v1–v2 usan el componente de la aplicación; Muro y Escalera son vistas de estudio sobre el mismo ViewModel.</p></div>
  </aside>;
}
