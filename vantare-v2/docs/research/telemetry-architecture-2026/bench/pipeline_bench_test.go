...
// BenchmarkStageProjectOverlay prices overlay.ProjectV1 alone.
func BenchmarkStageProjectOverlay(b *testing.B) {
	for _, count := range VehicleCounts {
		b.Run(fmt.Sprintf("vehicles=%d", count), func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				if _, err := overlay.ProjectV1(final); err != nil {
					b.Fatal(err)
				}
			}
		})
...
// BenchmarkMarshalOverlayV1 prices json.Marshal of the current payload.
func BenchmarkMarshalOverlayV1(b *testing.B) {
	for _, count := range VehicleCounts {
		b.Run(fmt.Sprintf("vehicles=%d", count), func(b *testing.B) {
			final := FinalStateFixture(b, count, 32)
...
// BenchmarkEndToEndCurrent runs reducer -> derive -> overlay projection ->
// marshal for one frame, i.e. the per-frame cost of the current design when
// only the overlay product is published.
func BenchmarkEndToEndCurrent(b *testing.B) {
	ctx := context.Background()
	for _, count := range VehicleCounts {
		b.Run(fmt.Sprintf("vehicles=%d", count), func(b *testing.B) {
			batches := make([]core.Batch, 256)
