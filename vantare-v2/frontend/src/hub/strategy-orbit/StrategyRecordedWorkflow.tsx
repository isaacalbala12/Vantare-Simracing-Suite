...
export function StrategyRecordedWorkflow({ eventId, repositoryVersion, repositoryLoading = false, onRetryRepository, initial, catalog, catalogState, calendar, application, analysis, onExit, onCleanupError, navigation, t }: {
  ...
  const backToPreparation = () => {
    if (flow.busy || formPending || flow.sessions.corrections.unresolved) return;
    flow.prepare();
  };
  const backBlocked = flow.busy || formPending || flow.sessions.corrections.unresolved;
  const sourceView = <StrategyRecordedSessionsView controller={{ ...flow.sessions, busy: flow.busy, locked: flow.sessions.locked || formPending }} onInspect={inspectSession} t={t} />;
