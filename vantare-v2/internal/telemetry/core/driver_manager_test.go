...
func TestDriverManagerResetsReconnectAttemptsAfterRecovery(t *testing.T) {
	// El contador solo se reiniciaba al arrancar y al parar, asi que sumaba
	// durante toda la vida de la aplicacion. Con desconexiones ocasionales a lo
	// largo de una sesion larga se agotaba MaxReconnects y el driver quedaba
	// terminal. Debe contar fallos seguidos, no totales.
	transient := errors.New("transient disconnect")
	constructed := 0
	sink := &collectingManagerSink{values: make(chan int, 2)}
	manager, err := NewDriverManager([]DriverCandidate[int]{
	...
	for {
		...
		if status.State == driver.StateLive && status.ReconnectAttempt == 0 {
			break
		}
		select {
		case <-deadline:
			t.Fatalf("reconnect attempts were not reset after recovery: %#v", status)
