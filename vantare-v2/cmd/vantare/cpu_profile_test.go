...
func newFakeCPUProfileControl() *fakeCPUProfileControl {
	return &fakeCPUProfileControl{stopped: make(chan struct{}, 8)}
}

func (fake *fakeCPUProfileControl) control() cpuProfileControl {
	return cpuProfileControl{
...
func TestParseCPUProfileDuration(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "not a duration", raw: "soon", err: errCPUProfileDurationUnparsable},
		{name: "bare number", raw: "30", err: errCPUProfileDurationUnparsable},
