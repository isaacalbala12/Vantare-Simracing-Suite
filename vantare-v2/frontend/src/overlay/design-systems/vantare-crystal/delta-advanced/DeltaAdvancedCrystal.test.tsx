...
describe("DeltaAdvancedCrystal", () => { it("renders the four canonical section 16 cells without inventing unavailable values", () => { const { container } = render(<DeltaAdvancedCrystal model={model} settings={{}} renderMode="harness" />); const root = container.querySelector('[data-widget-system="vantare-crystal"]') as HTMLElement; expect([...root.querySelectorAll("i")].map((node) => node.textContent)).toEqual(["B", "S", "T", "L"]); expect([...root.querySelectorAll(":scope > div")].map((node) => node.getAttribute("data-tone"))).toEqual(["b", "s", "t", "l"]); expect(root.textContent).toContain("-0.150"); expect(root.textContent?.match(/--\.---/g)).toHaveLength(3); }); });
