...
export function StrategyRecordedPlan({ draft, state, acceptance, locked, onChange, onCalculate, onCancel, t }: {
  ...
  return <section className="strategy-recorded-plan" aria-labelledby="recorded-plan-title">
    ...
    {acceptance.state.status === "accepted" ? <p role="status">{t("strategy.plan.accepted")}</p> : null}
    ...
    <footer className="strategy-recorded-plan__actions">
      ...
      {plan ? <Button variant="primary" disabled={locked || accepting || acceptance.state.status === "accepted" || acceptance.state.status === "recovery" || acceptance.state.status === "error"} onClick={() => void acceptance.accept()}>{t(accepting ? "strategy.plan.accepting" : "strategy.plan.accept")}</Button> : null}
    </footer>
  </section>;
}
