...
func TestStrategySignalAuditSupportedRowsMatchProductionContracts(t *testing.T) {
	...
	if MatrixVersion != 6 || len(matrix) != 38 || len(rules) != 38 {
		t.Fatalf("runtime authority matrix = version %d / %d rules / %d unique, want v6 / 38 / 38", MatrixVersion, len(matrix), len(rules))
	}
...
func TestStrategySignalAuditV1HasExactReviewedProductionSurfaces(t *testing.T) {
	for _, current := range []struct {
		...
		{name: "core.ObservedState", typeOf: reflect.TypeOf(telemetrycore.ObservedState{}), fields: []string{
			"SourceTime", "EndTime", "MaximumLaps", "TrackName", "SessionType", "VehicleCount", "PlayerPresent", "AmbientTemp", "TrackTemp", "RainFraction", "WetnessFraction", "SessionFlag", "Vehicles",
		}},
