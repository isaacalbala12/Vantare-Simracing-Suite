...
Plan: declarar retirada en la definición; filtrar catálogos y actualizar etiquetas visibles; probar perfiles mixtos V3/V4 con ajustes, memorias, procedencia y políticas; verificar selector, renderer antiguo, idiomas, frontend y revisión visual. Mantener manifest, parser, backend y IDs de diseños antiguos para resolver perfiles existentes. Sin merge ni cambio al Workshop 5178.

## Ampliación aprobada: incorporar la revisión de datos

El orquestador solicita incorporar como tercer commit en #1356 el parche auditado `fb859087`, preparado por el agente de datos sobre la misma base `f50ab4ab`. Se revisan su diff, los cinco casos sobre el fixture de producción y los consumidores de los helpers compartidos antes de aplicar un cherry-pick con procedencia conservada.

El ViewModel distingue ceros válidos de señales ausentes o inválidas; limita los pedales a 0–1; vacía datos anteriores mientras la fuente conecta, detecta o se detiene; y calcula la calidad sin penalizar un embrague oculto ni la dirección opcional. El compacto también usa la disponibilidad de los instrumentos compartidos; Input Telemetry conserva su conversión común de velocidad, que ahora descarta valores no finitos. La misma normalización de pedales se reutiliza en el ViewModel Pedals existente, con reglas idénticas de calidad, finitud y límites; se elimina una copia detectada por el ratchet sin modificar la política. No cambia el backend, la persistencia ni la presentación del selector.

Verificación conjunta: casos focales del ViewModel, compacto, Input Telemetry, perfiles antiguos, selector y paridad; suite frontend completa, tipos, lint, build y ratchet. La señal física de dirección sigue ausente en producción y la posición V2 sigue sin dato: integrar este parche no certifica LMU activo ni el ángulo del volante.
