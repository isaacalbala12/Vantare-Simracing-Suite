...
export function StrategyRecordedWorkflow({ eventId, repositoryVersion, repositoryLoading = false, onRetryRepository, initial, catalog, catalogState, calendar, application, analysis, onExit, onCleanupError, navigation, t }: {
  ...
  const selectedSessions = flow.sessions.sessions.filter(session => flow.draft.sessions.some(ref => ref.sessionId === session.revision.sessionId));
  ...
  const sourceView = <StrategyRecordedSessionsView controller={{ ...flow.sessions, locked: flow.sessions.locked || formPending }} t={t} />;
