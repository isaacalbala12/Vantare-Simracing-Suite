...
- Evidencia re-review fresca: focal core/driver/LMU repetido 20 veces PASS; fuzz explícito 10 s PASS (~603.012 ejecuciones); benchmark estabilidad+parse+evidencia por muestra 23,25–23,83 µs/op, 592 B/op y 14 allocs/op (margen >690x a 60 Hz); `go test ./internal/telemetry/... -count=1`, guard ADR y `git diff --check` PASS. Vet core/driver PASS; LMU conserva dos warnings localizados `unsafe.Pointer` al materializar mmap y VS_FIXEDFILEINFO Win32. `-race` se intentó y no arranca con `CGO_ENABLED=0`.
