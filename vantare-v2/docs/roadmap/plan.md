...
- cuerpo: Paridad CrewChief por cortes T0–T8. T0a y T0b tienen referencias y casos independientes revisados; T1 aporta un cálculo sintético de relaciones de clase, pista y tiempos de paso, desactivado por defecto y aún sin conexión a la voz. Faltan verificar las señales LMU reales y completar muestreo, silencios, cadencia, consultas y escucha. Los hechos seguirán siendo deterministas; el LLM cloud redactará con StyleGate y el fallback offline usará frases locales precacheadas. Este avance no acredita paridad funcional completa.
- cuerpo.en: CrewChief parity in T0–T8 stages. T0a and T0b have reviewed references and independent cases; T1 adds synthetic class, track and passage-time relations, disabled by default and not yet connected to voice. Real LMU signals, sampling, silence rules, cadence, queries and listening remain to be verified. Facts remain deterministic; the cloud LLM will write behind a StyleGate and offline fallback will use local precached phrases. This progress does not establish full functional parity.
- cuerpo.pt: Paridade CrewChief em etapas T0–T8. T0a e T0b têm referências e casos independentes revistos; T1 acrescenta relações sintéticas de classe, pista e tempos de passagem, desativadas por padrão e ainda sem ligação à voz. Falta verificar sinais LMU reais e concluir amostragem, silêncios, cadência, consultas e escuta. Os factos continuam determinísticos; o LLM cloud redigirá com StyleGate e o fallback offline usará frases locais pré-armazenadas. Este avanço não comprova paridade funcional completa.
- cuerpo.it: Parità CrewChief in fasi T0–T8. T0a e T0b dispongono di riferimenti e casi indipendenti revisionati; T1 aggiunge relazioni sintetiche di classe, pista e tempi di passaggio, disattivate per impostazione predefinita e non ancora collegate alla voce. Restano da verificare i segnali LMU reali e completare campionamento, silenzi, cadenza, query e ascolto. I fatti restano deterministici; il LLM cloud scriverà con StyleGate e il fallback offline userà frasi locali in cache. Questo avanzamento non dimostra la piena parità funzionale.
- etiqueta: Plan
- etiqueta.pt: Plano
- etiqueta.it: Piano
