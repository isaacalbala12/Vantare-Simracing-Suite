...
- Worker con assets siempre tras verificación criptográfica de Cloudflare Access, correo único y host exacto; sin ruta `workers.dev`. Supabase entrega solo agregados de cuentas/dispositivos y registro semanal manual. Polar Metrics API es la autoridad para MRR, suscripciones y cobros, separando producción y sandbox. La bandeja de opiniones depende de VAN-757; el retorno D7–13 continúa sin medir. Las fuentes fallidas se marcan «no disponibles».
