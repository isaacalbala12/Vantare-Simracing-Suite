...
function createRouteScenarioWidget(query: OverlayWorkshopQuery): WidgetInstanceV3 {
  ...
  // aplican a standings Endurance — el renderer las revalida antes de dibujar.
...
function OverlayWorkshopPage({ initialQuery, initialError, profileId }: { initialQuery: OverlayWorkshopQuery; initialError?: string; profileId: string }): React.ReactElement {
  ...
  // → módulos → pie de datos); encima, createRouteScenarioWidget aplica el
  // laboratorio tower de Redline como appearanceOverrides. Sin preparación
  // diferida — lo que la URL dice es lo que se renderiza, y una combinación
  // inválida cae al error visible con los controles vivos.
  const built = useMemo(() => {
    try {
      return { widget: createRouteScenarioWidget(parsed), error: null as string | null };
    } catch (error) {
  ...
  const showDesign = () => {
    setPlaying(false);
    setAuthoringPlayback(false);
    if (parsed.surface !== "studio" && parsed.surface !== "harness") {
      update({ ...parsed, surface: "studio", compare: parsed.compare === "studio" ? undefined : parsed.compare });
    }
  };
