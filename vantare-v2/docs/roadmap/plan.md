...
- cuerpo: Campaña aprobada para reducir CPU, GPU y memoria y acelerar arranque, apertura de pantallas, navegación y respuesta de la base de Vantare, conservando apariencia y capacidades. Incluye Hub, navegación y servicios comunes; HUD y Overlay Studio quedan excluidos. El escenario principal mantiene el simulador abierto y mide los procesos propios de Vantare. Corte 4 entregado: cada pantalla del Hub carga bajo demanda y el JS que el Hub descarga al arrancar baja de ~2.263 kB a ~1.241 kB medidos en build (el chunk de la shell, de 1.054 a 69 kB); el CSS de cada página viaja en su propio chunk. El ahorro de CPU/RAM sigue pendiente del banco Windows.
- cuerpo.en: Approved campaign to reduce CPU, GPU and memory use and speed up startup, screen opening, navigation and interaction in the Vantare base app while preserving appearance and capabilities. Includes the Hub, navigation and shared services; HUD and Overlay Studio are excluded. The main scenario keeps the simulator open and measures Vantare-owned processes. Cut 4 delivered: every Hub screen loads on demand and the JS the Hub downloads at startup drops from ~2,263 kB to ~1,241 kB as measured in the build (the shell chunk, from 1,054 to 69 kB); each page's CSS travels in its own chunk. CPU/RAM savings remain pending the Windows bench.
- cuerpo.pt: Campanha aprovada para reduzir CPU, GPU e memória e acelerar o arranque, a abertura de ecrãs, a navegação e a resposta da base do Vantare, preservando aparência e capacidades. Inclui Hub, navegação e serviços comuns; HUD e Overlay Studio ficam excluídos. O cenário principal mantém o simulador aberto e mede os processos do Vantare. Corte 4 entregue: cada ecrã do Hub carrega sob demanda e o JS que o Hub descarrega no arranque desce de ~2.263 kB para ~1.241 kB medidos no build (o chunk da shell, de 1.054 para 69 kB); o CSS de cada página viaja no seu próprio chunk. A poupança de CPU/RAM continua pendente do banco Windows.
- cuerpo.it: Campagna approvata per ridurre CPU, GPU e memoria e accelerare avvio, apertura delle schermate, navigazione e risposta della base di Vantare, preservando aspetto e capacità. Include Hub, navigazione e servizi comuni; HUD e Overlay Studio sono esclusi. Lo scenario principale mantiene aperto il simulatore e misura i processi di Vantare. Taglio 4 consegnato: ogni schermata dell'Hub si carica su richiesta e il JS scaricato dall'Hub all'avvio scende da ~2.263 kB a ~1.241 kB misurati in build (il chunk della shell, da 1.054 a 69 kB); il CSS di ogni pagina viaggia nel proprio chunk. Il risparmio di CPU/RAM resta in attesa del banco Windows.
- etiqueta: ISA-1015
