...
it("round-trips driver limits in contract units", () => {
  const draft = { ...payload.draft, drivers: [{ id: "a", name: "Alex" }], rules: { driverLimits: { a: { minLaps: 12, maxLaps: 40, maxContinuousTimeSeconds: 1800, maxTotalTimeSeconds: 5400 } } } };
  expect(parseRecordedDraftPayload({ ...payload, draft }).draft).toEqual(draft);
});
