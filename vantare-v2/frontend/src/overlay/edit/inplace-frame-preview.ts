import type { WidgetLayoutV3 } from "../core/profile-document";
import { resolveWidgetVisualGeometry } from "../core/widget-visual-geometry";
...
export function inplaceFrameTestId(widgetId: string): string {
  return `inplace-edit-frame-${widgetId}`;
}

export function registerInplaceFrameElement(widgetId: string, element: HTMLElement | null): void {
  if (element) {
...
export function findInplaceFrameElement(widgetId: string): HTMLElement | null {
  return frameElements.get(widgetId) ?? document.querySelector<HTMLElement>(`[data-testid="${inplaceFrameTestId(widgetId)}"]`);
}

export function getInplaceFrameLayoutPreview(widgetId: string): WidgetLayoutV3 | undefined {
  return previewSessions.get(widgetId)?.preview;
}

export function clearInplaceFrameLayoutPreview(widgetId: string): void {
  const frame = findInplaceFrameElement(widgetId);
  if (frame) {
...
export function beginInplaceFramePreview(
  widgetId: string,
  kind: FramePreviewKind,
  start: WidgetLayoutV3,
): void {
...
export function applyInplaceFrameLayoutPreview(
  widgetId: string,
...
export function resetInplaceFrameLayoutPreview(
  widgetId: string,
  layout: WidgetLayoutV3,
): void {
  const session = previewSessions.get(widgetId);
  ...
  clearInplaceFrameLayoutPreview(widgetId);
}

export function resolveInplaceFrameGeometry(
  widgetId: string,
  layout: WidgetLayoutV3,
