...
- **Stop conditions:** `Reset()` global para un toggle, cancelación de cola completa o API nueva en `audio/queue.go`, versionado nuevo de sensibilidad/evidence, modificar `spotter_policy.go`/`types.go` o `scheduler.go` fuera del cambio mínimo autorizado (reset aditivo/scoped de política Spotter), aserto legacy rojo sin remedio autorizado (STOP y revisión; sin filtro ni cambio lógico en `ConsumeObservation`),
  ...
  activos/histéresis topológicos del baseline (diferido a S2). Antes de editar, revalidar Nightly: detenerse solo si hay deriva funcional nueva en `internal/engineer`/`cmd/vantare` respecto al baseline `8880a880`; el SHA de `origin/nightly` es observación, no autoridad fija (observación 2026-08-12: `cc54d3614c5749ee7a014274ba3598365fec477c`, sin deriva Engineer); no se hace rebase ahora.
...
- **MODIFICAR (mínimo):** `service/engineer_service.go` — wiring mínimo de locale antes de `Start` que deriva el locale efectivo del `AudioConfig` ya inyectado (`Lang(Channel)` de `audio/config.go`, read-only), sin hardcodear otro mapa ni segunda fuente de verdad, para el default coherente por canal/familia (Spotter EN, Engineer ES; presentación y audio mismo locale o fallo
...
- **SOLO LEER / contexto:** `audio/config.go` (`DefaultAudioConfig`) y `audio/config_test.go` (`TestDefaultAudioConfig`) — se conservan defaults y test actuales; `cmd/vantare/main.go` — composición,
  ...
  `EngineerService.SetLocale`, `presentation.ParseLocale` y `presentation.Resolver` — SOLO LEER/intactos; `ParseLocale` sigue siendo la autoridad de parsing y soporta es/en/it/pt-BR (S1 no cambia parsing); el locale efectivo se deriva de `AudioConfig.Lang(Channel)` ya inyectado; el wiring usa solo los defaults Spotter EN / Engineer ES;
