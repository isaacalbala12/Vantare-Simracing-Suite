...
import type { BroadcastTowerViewModel } from "./broadcast-tower-view-model";

export type BroadcastTowerContent = { rowCount: number; showWeather: boolean; showSof: boolean };
const DEFAULT_CONTENT: BroadcastTowerContent = { rowCount: 5, showWeather: true, showSof: false };
...
export const broadcastTowerDefinition: WidgetTypeDefinition<BroadcastTowerContent, BroadcastTowerViewModel> = {
  ...
  capabilities: { inspectorSections: ["design", "appearance", "content", "behavior", "layout", "actions"], supportsAspectUnlock: true, resizeMode: "horizontal-only", minimumSize: { width: 340, height: DEFAULT_LAYOUT.h }, defaultSize: { width: DEFAULT_LAYOUT.w, height: DEFAULT_LAYOUT.h }, requiredFeature: getWidgetRequiredFeature("broadcast-tower") }, inspector,
