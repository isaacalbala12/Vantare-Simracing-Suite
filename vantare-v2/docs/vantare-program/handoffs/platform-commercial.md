...
## VAN-758 / ISA-1345 — consola privada de inteligencia (2026-09-23)

- [Tarea Notion](https://app.notion.com/p/3e4e51695c658131b264d34a367c0af9), [issue puente #1345](https://github.com/isaacalbala12/Vantare-Simracing-Suite/issues/1345), rama `vantareapp/isa-1345-inteligencia-web` desde `nightly@8b25d076`. Isaac aprobó una web privada en Cloudflare para producto, Polar, opiniones, mercado y crecimiento; es un proyecto de una persona.
- Worker con assets siempre tras verificación criptográfica de Cloudflare Access, correo único y host exacto; sin ruta `workers.dev`. Supabase entrega solo agregados de cuentas/dispositivos y registro semanal manual, incluidas cohortes maduras D7–13 con retornos confirmados y desconocidos. Sin recuentos válidos, el retorno sigue «sin medir»; con ellos se muestra una cota mínima y su cobertura, no una extrapolación automática. Polar Metrics API es la autoridad para MRR, suscripciones y cobros, separando producción y sandbox. La bandeja de opiniones depende de VAN-757. Las fuentes fallidas se marcan «no disponibles».
- La migración de Supabase y el despliegue Cloudflare **no** se han aplicado. El primer acceso requiere dominio, correo, configuración de Access, clave de servidor Supabase y tokens de lectura Polar. Ningún secreto pertenece al repo. El procedimiento y comprobaciones están en `infra/intelligence-web/README.md`.
- Roadmap: `phases:ecosystem`; el trabajo prepara la medición interna sin afirmar que haya sesiones de simulación o ingresos comprobados. Mercado potencial: todo el simracing en la comunicación, con mercado servible ligado a los simuladores realmente compatibles.
- Siguiente paso: completar revisión local/CI y PR draft a nightly; después revisar dominio y Access con Isaac antes de activar la web. No promover a Testers/Master ni publicar cifras sin fuentes verificadas.
