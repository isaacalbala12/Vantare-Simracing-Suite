...
- TDD cubre orden, duplicados, gaps de secuencia/epoch, reset, lote todo-o-nada, identidad, copia defensiva de entrada/salida, mutación concurrente, determinismo, lifecycle/cancelación y owner único. Incluye fuzz de cursores y benchmark de copia completa con 64 vehículos.
- Decisiones y guía de verificación: `docs/telemetry-core/runtime-reducer.md`. Evidencia fresca: core x20, Telemetry Core completo, guard ADR 0004, vet focal, frontend build para generar `dist` ignorado y suite global Go PASS; fuzz 10 s PASS con 2.318.048 ejecuciones. Benchmark Windows/amd64 con 64 vehículos, cinco repeticiones: 8,11–9,26 µs/op, 36.264 B/op y 5 allocs/op. El vet amplio conserva solo tres warnings `unsafe.Pointer` heredados en readers/version Win32 fuera del diff. `-race` no está disponible: el entorno usa `CGO_ENABLED=0` y con CGO habilitado no encuentra `gcc`.
