...
- **Tests NUEVOS (toggle, no SetOutputMode):** `service/engineer_service_test.go` — `TestSetSpotterEnabledCancelsOnlySpotterAndPreservesFuel`, `TestSetSpotterEnabledRearmsCleanState` (tras re-enable no reaparece ningún mensaje Spotter legacy previo al disable, preservando Fuel) y "toggle Spotter preserva el visual activo Fuel/Engineer"
...
- **Orden TDD red-verde:** (1) rojo: re-enable conserva estado Spotter obsoleto (still/clear tardío o mensaje legacy reemitido) porque el toggle no limpia la máquina; (2) rojo:
