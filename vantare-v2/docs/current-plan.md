...
- Compatibilidad/lifecycle fail-closed: `known` exige build allowlisted `1.3.0.0` y evidencia estructural positiva. El provider Windows localiza el proceso sin ruta hardcodeada, mantiene la ruta privada y lee FileVersion/ProductVersion con Win32/version.dll una vez por `Run`; ausencia/no-Windows/no allowlist queda `unknown/degraded` sin campos fresh. Menú valida scoring y puede estar live sin vehículo; jugador exige correlación scoring/telemetry por isPlayer, ID, vehículo y track. Telemetry movida/corrupta queda `unknown` aunque scoring sea plausible.
...
- Evidencia re-review fresca: focal core/driver/LMU repetido 20 veces PASS; fuzz explícito 10 s PASS (~571.962 ejecuciones); benchmark estabilidad+parse+evidencia por muestra 23,18–24,72 µs/op, 592 B/op y 14 allocs/op (margen >670x a 60 Hz); `go test ./internal/telemetry/... -count=1`, guard ADR y `git diff --check` PASS. Vet core/driver PASS; LMU conserva dos warnings localizados `unsafe.Pointer` al materializar mmap y VS_FIXEDFILEINFO Win32. `-race` se intentó y no arranca con `CGO_ENABLED=0`.
- Live opt-in read-only con LMU abierto: provider/version PASS (`1.3.0.0` allowlisted), compatibilidad FAIL `evidence=scoring-insufficient`; no se capturó/escribió raw. El gate queda pendiente de repetir cuando LMU publique scoring completo; no se relaja fail-closed para forzar PASS.
