...
func TestPlayerScriptMediaEvents(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			prefix := fmt.Sprintf(`$ErrorActionPreference = 'Stop';
[Console]::OutputEncoding = New-Object System.Text.UTF8Encoding;
Add-Type -AssemblyName WindowsBase;
Add-Type -TypeDefinition @'
%s
'@ -ReferencedAssemblies ([System.Windows.Threading.Dispatcher].Assembly.Location);
[MediaEventsFixture]::Scenario = '%s';
[MediaEventsFixture]::ExpectedPath = '%s';
`, source, tc.name, escapePSQuote(path))
