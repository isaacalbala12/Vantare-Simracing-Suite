...
- `buildEngineerRadioViewModel(_content, runtime)` ya no recibe `TelemetrySnapshot`. La única entrada real es `runtime.engineerPresentation: EngineerPresentation | undefined` y `runtime.engineerSubtitlesEnabled`.
...
**Evidencia**:
- `engineer-radio-definition.ts` — `runtime.engineerPresentation` como única fuente productiva.
