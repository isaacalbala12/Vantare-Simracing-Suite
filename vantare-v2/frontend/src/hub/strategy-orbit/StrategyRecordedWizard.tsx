    ...
    <fieldset disabled={busy}>
      <StrategyRecordedFrame steps={RECORDED_WIZARD_STEPS.map((id, position) => ({ id, label: t(`strategy.journey.step.${id}`), available: position <= index }))}
        ...
        actions={<>
          ...
          <button type="submit" className="orbit-btn orbit-btn--primary">{t(busy ? "strategy.journey.opening" : draft.step === "sessions" ? "strategy.journey.openDraft" : "strategy.journey.next")} →</button>
        </>}
      >
