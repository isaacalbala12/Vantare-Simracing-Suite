...
Deno.test("official deploy workflow can only deploy through the guarded wrapper", () => {
  const wrapper = Deno.readTextFileSync(new URL("./deploy-approved-functions.ps1", import.meta.url));
  ...
  if (guardIndex < 0 || deployIndex < 0 || guardIndex >= deployIndex) {
    throw new Error("deploy wrapper does not enforce the surface guard before deployment");
  }
  ...
  if (workflow.includes("supabase functions deploy")) {
    throw new Error("official workflow contains an unguarded direct deploy command");
  }
  if (!workflow.includes("supabase/setup-cli@3c2f5e2ae34c34e428e8e206e2c4d21fa2d20fbf")) {
    throw new Error("official workflow does not pin the supported Supabase setup action");
  }
  if (workflow.includes("npm install --global supabase")) {
    throw new Error("official workflow uses the unsupported global npm installation");
  }
});
