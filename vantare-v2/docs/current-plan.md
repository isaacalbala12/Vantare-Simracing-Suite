...
- `Status` consulta el snapshot del driver fuera del mutex, valida cada cambio contra `driver.State.CanTransitionTo` y persiste solo transiciones legales. Un salto o retroceso ilegal publica `degraded` con `ErrInvalidDriverTransition`; una recuperación legal limpia el diagnóstico.
