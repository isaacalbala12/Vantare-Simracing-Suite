...
- id: radar-proximity
- tipo: feature
- titulo.en: Proximity radar
- titulo.pt: Radar de proximidade
- titulo.it: Radar di prossimità
- cuerpo: El radar Eficiencia dibuja hasta 16 coches a 30 metros alrededor del jugador con posiciones espaciales frescas de LMU, orientadas según su coche. Destaca los coches en paralelo mediante el criterio geométrico del Spotter, se oculta cuando faltan datos fiables y usa un widget transparente compartido entre Studio, Desktop, OBS y Workshop. La demostración de Workshop usa posiciones de ejemplo. La comprobación física en una sesión LMU activa queda pendiente.
- cuerpo.en: The Efficiency radar draws up to 16 cars within 30 metres of the player from fresh LMU spatial positions, aligned to the player's car. It highlights overlapping cars using Spotter geometry, hides when reliable data is unavailable, and shares one transparent widget across Studio, Desktop, OBS and Workshop. The Workshop preview uses example positions. Physical validation in a live LMU session remains pending.
- cuerpo.pt: O radar Eficiência mostra até 16 carros a 30 metros do jogador com posições espaciais recentes do LMU, alinhadas com o carro do jogador. Destaca carros lado a lado com a geometria do Spotter, oculta-se quando faltam dados fiáveis e partilha um widget transparente entre Studio, Desktop, OBS e Workshop. A pré-visualização do Workshop usa posições de exemplo. A validação física numa sessão LMU ativa permanece pendente.
- cuerpo.it: Il radar Efficienza mostra fino a 16 auto entro 30 metri dal giocatore usando posizioni spaziali recenti di LMU, allineate alla sua auto. Evidenzia le auto affiancate con la geometria dello Spotter, si nasconde quando mancano dati affidabili e condivide un widget trasparente tra Studio, Desktop, OBS e Workshop. L'anteprima Workshop usa posizioni di esempio. La verifica fisica in una sessione LMU attiva resta in sospeso.
- etiqueta: ISA-1388
