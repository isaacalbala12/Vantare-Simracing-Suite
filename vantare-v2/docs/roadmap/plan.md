...
- cuerpo: Campaña aprobada para reducir CPU, GPU y memoria y acelerar arranque, apertura de pantallas, navegación y respuesta de la base de Vantare, conservando apariencia y capacidades. Incluye Hub, navegación y servicios comunes; HUD y Overlay Studio quedan excluidos. El candidato conjunto centraliza los eventos Wails de ajustes, licencia y actualización, cancela trabajo al desmontar y reduce a 30 segundos el reloj de próximas carreras lejanas. El escenario principal mantiene el simulador abierto y mide los procesos propios de Vantare. Todavía no se anuncia ahorro medido. Además, comparte las suscripciones de overlays y calendario, carga las páginas del Hub y los diccionarios secundarios bajo demanda, y aplica el presupuesto de efectos `noBlur`/`flat` en la shell.
- cuerpo.en: Approved campaign to reduce CPU, GPU and memory use and speed up startup, screen opening, navigation and interaction in the Vantare base app while preserving appearance and capabilities. It includes the Hub, navigation and shared services; HUD and Overlay Studio are excluded. The joint candidate centralizes Wails settings, license and updater events, cancels work on unmount and slows the distant upcoming-race clock to 30 seconds. The main scenario keeps the simulator open and measures Vantare-owned processes. No measured savings are announced yet. It also shares overlay and calendar subscriptions, loads Hub pages and secondary language dictionaries on demand, and applies the `noBlur`/`flat` effects budget to the shell.
- cuerpo.pt: Campanha aprovada para reduzir CPU, GPU e memória e acelerar o arranque, a abertura de ecrãs, a navegação e a resposta da base do Vantare, preservando aparência e capacidades. Inclui Hub, navegação e serviços comuns; HUD e Overlay Studio ficam excluídos. O candidato conjunto centraliza os eventos Wails de definições, licença e atualização, cancela trabalho ao desmontar e reduz para 30 segundos o relógio de corridas futuras distantes. O cenário principal mantém o simulador aberto e mede os processos do Vantare. Ainda não se anuncia uma redução medida. Também partilha as subscrições de overlays e calendário, carrega as páginas do Hub e os dicionários de idiomas secundários sob demanda e aplica o orçamento de efeitos `noBlur`/`flat` à shell.
- cuerpo.it: Campagna approvata per ridurre CPU, GPU e memoria e accelerare avvio, apertura delle schermate, navigazione e risposta della base di Vantare, preservando aspetto e capacità. Include Hub, navigazione e servizi comuni; HUD e Overlay Studio sono esclusi. Il candidato congiunto centralizza gli eventi Wails di impostazioni, licenza e aggiornamento, annulla il lavoro allo smontaggio e porta a 30 secondi l'orologio delle gare lontane. Lo scenario principale mantiene aperto il simulatore e misura i processi di Vantare. Non viene ancora annunciato alcun risparmio misurato. Condivide inoltre le sottoscrizioni di overlay e calendario, carica su richiesta le pagine Hub e i dizionari delle lingue secondarie e applica alla shell il budget effetti `noBlur`/`flat`.
- etiqueta: ISA-1015
...
- cuerpo: Studio reconoce documentos guardados equivalentes; las tablas Crystal respetan sus columnas y ajustan altura al cambiar filas. Pedales conservan el historial y muestran R/N, colores configurados y escala RPM sin simular el corte del coche. Track Map tiene fondo transparente y leyenda de clases. Las comparaciones explican referencias y datos ausentes; Head-to-Head protege su geometría. Dense contiene los tres pedales y el historial a 560×100; Broadcast omite límites de vueltas inválidos o ilimitados. El ciclo de vida de la ventana overlay de escritorio serializa abrir y detener: un arranque concurrente no deja ventanas huérfanas y detener durante la creación no deja reaparecer el overlay. La validación física aislada con Clean Overlay está completada; queda la comprobación conjunta con Efficiency.
- cuerpo.en: Studio recognizes equivalent saved documents; Crystal tables honor columns and adjust height when row count changes. Pedals preserve history and show R/N, configured colors and an RPM scale without simulating the car redline. Track Map has a transparent background and class legend. Comparisons explain references and missing data; Head-to-Head protects its geometry. Dense contains all three pedals and history at 560×100; Broadcast omits invalid or unlimited lap limits. The desktop overlay window lifecycle serializes open and stop: a concurrent start no longer leaves orphaned windows and stopping during creation does not let the overlay resurrect. Isolated physical validation with Clean Overlay is complete; joint validation with Efficiency remains.
- cuerpo.pt: Studio reconhece documentos guardados equivalentes; tabelas Crystal respeitam colunas e ajustam a altura ao mudar as linhas. Pedais preservam o histórico e mostram R/N, cores configuradas e escala RPM sem simular o limite do carro. Track Map tem fundo transparente e legenda de classes. Comparações explicam referências e dados ausentes; Head-to-Head protege a geometria. Dense contém os três pedais e o histórico a 560×100; Broadcast omite limites de voltas inválidos ou ilimitados. O ciclo de vida da janela overlay de ambiente de trabalho serializa abrir e parar: um arranque concorrente não deixa janelas órfãs e parar durante a criação não deixa o overlay reaparecer. A validação física isolada com Clean Overlay está concluída; falta a validação conjunta com Efficiency.
- cuerpo.it: Studio riconosce documenti salvati equivalenti; le tabelle Crystal rispettano le colonne e adattano l'altezza al numero di righe. I pedali conservano la cronologia e mostrano R/N, colori configurati e una scala RPM senza simulare il limitatore dell'auto. Track Map ha sfondo trasparente e legenda di classe. I confronti spiegano riferimenti e dati assenti; Head-to-Head protegge la geometria. Dense contiene tutti e tre i pedali e la cronologia a 560×100; Broadcast omette limiti di giri non validi o illimitati. Il ciclo di vita della finestra overlay desktop serializza apertura e arresto: un avvio concorrente non lascia finestre orfane e arrestare durante la creazione non fa riapparire l'overlay. La verifica fisica isolata con Clean Overlay è completa; resta la verifica congiunta con Efficiency.
