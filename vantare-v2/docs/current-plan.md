Nota ISA-765 / ISA-694 F5-b (2026-08-22, implementada en rama de issue):
- Strategy consulta a Analysis los derivados de la combinación usando solo las
  ...
  TypeScript.
- El documento canónico v2 guarda a la vez la proyección y overrides manuales o
  de referencia por campo. El cálculo elige el override válido; revertirlo
  elimina solo el override y recupera el derivado sin destruir su muestra,
  rango ni procedencia.
- Orbit muestra chips Derivado, Manual, Referencia y Falta para los datos
  ...
  Go/frontend, typecheck, build y protocolo visual quedan verdes. Sin PR,
  integración, promoción ni release; sigue pendiente F5-e para conectar la
  fuente autorizada inicial.
...
- El clima se condiciona solo con eventos `Minimum Path Wetness`: 0 % seco,
  hasta 5 % húmedo y más de 5 % mojado. Una vuelta que cambia de bucket no se
  atribuye entera a ninguno. `CloudDarkness` y `OffpathWetness` no participan.
  Tráfico conserva consumo y etiqueta, pero queda fuera de la mediana de ritmo.
...
- Commits de prueba/producto: `a714ac1c`, `032b2077`, `ed856869` y
  `9829dfd8`. Sin PR, merge, promoción ni release. Siguiente: review del
  orquestador de #742; F3-a4 no se asume iniciada.
