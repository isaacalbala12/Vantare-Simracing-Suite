...
| **prioridad propuesta** | Orden total del bus (spec §4.1): **P0 spotter** (`PrioritySpotter`) > **P1 banderas/peligro** (`PriorityRaceControl`) > **P2 fuel/pit crítico** (`PriorityFailureResource` / `PriorityStrategyUrgent`) > **P3 carrera** (`PriorityPenalty` / `PriorityInformation` según subdominio) > **P4 rendimiento/motivación** (`PriorityMotivation`). Mapeo exacto a `messagepolicy.Priority`. |
...
| `fuel.consumption_report` | Consumo {n} por vuelta · Consumo medio {n} litros por vuelta | Consumption {n} per lap · Average {n} litres per lap | Consumo {n} al giro · Consumo medio {n} litri al giro | Consumo {n} por volta · Consumo médio {n} litros por volta | consulta | P4 `Motivation` | 20 s | `derive.FuelUsage.PerLap` (no proyectado aún) · **parcial** |
...
| intent | es (visual · voz) | en (visual · voz) | it (visual · voz) | pt-BR (visual · voz) | tipo | prioridad | TTL | señal canónica + disponibilidad |
|---|---|---|---|---|---|---|---|---|
| `conditions.track_temp_high` | Pista muy caliente · Temperatura de pista alta, {n} grados | Track very hot · Track temperature high, {n} degrees | Pista molto calda · Temperatura pista alta, {n} gradi | Pista muito quente · Temperatura da pista alta, {n} graus | informa | P4 `Motivation` | 30 s | `weather.Temperature` pista · **ausente** |
| `conditions.track_freezing` | Pista fría · Temperatura de pista baja, {n} grados | Track cold · Track temperature low, {n} degrees | Pista fredda · Temperatura pista bassa, {n} gradi | Pista fria · Temperatura da pista baixa, {n} graus | informa | P4 | 30 s | idem · **ausente** |
...
## 13. Familia strategy / motivación — P4, valor informativo
