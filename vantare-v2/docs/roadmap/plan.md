...
- cuerpo: Un banco Windows mide RAM, CPU y GPU por proceso, atribuye Hub y overlay con PID confirmados por CDP, excluye muestras GPU inválidas, recupera sesiones ETW huérfanas y solo publica frametime de LMU cuando PresentMon entrega frames válidos. ISA-978 añade evidencia experimental reproducible en Mac, un informe ciego congelado y una entrada de captura Windows que reutiliza el banco; startup completo, OBS físico y soak conservan gates explícitos sin resultados inventados. Telemetry Core queda fuera de valoración mientras está en desarrollo.
- cuerpo.en: A Windows bench measures RAM, CPU and GPU per process, attributes Hub and overlay with CDP-confirmed PIDs, excludes invalid GPU samples, recovers orphaned ETW sessions, and only publishes LMU frame time when PresentMon provides valid frames. ISA-978 adds reproducible Mac experiments, a frozen blind report and a Windows capture entry reusing this bench; complete startup, physical OBS and soak retain explicit gates without invented results. Telemetry Core is excluded from assessment while in development.
- cuerpo.pt: Um banco Windows mede RAM, CPU e GPU por processo, atribui Hub e overlay com PIDs confirmados por CDP, exclui amostras GPU inválidas, recupera sessões ETW órfãs e só publica o tempo de frame do LMU quando o PresentMon fornece frames válidos. ISA-978 adiciona experiências reproduzíveis no Mac, um relatório cego congelado e uma entrada de captura Windows que reutiliza o banco; startup completo, OBS físico e soak mantêm gates explícitos sem resultados inventados. Telemetry Core fica fora da avaliação durante o desenvolvimento.
- cuerpo.it: Un banco Windows misura RAM, CPU e GPU per processo, attribuisce Hub e overlay con PID confermati da CDP, esclude i campioni GPU non validi, recupera le sessioni ETW orfane e pubblica il frame time di LMU solo quando PresentMon fornisce frame validi. ISA-978 aggiunge esperimenti riproducibili su Mac, un rapporto cieco congelato e un punto di acquisizione Windows che riutilizza il banco; startup completo, OBS fisico e soak mantengono gate espliciti senza risultati inventati. Telemetry Core è escluso dalla valutazione durante lo sviluppo.
- etiqueta: Feature
