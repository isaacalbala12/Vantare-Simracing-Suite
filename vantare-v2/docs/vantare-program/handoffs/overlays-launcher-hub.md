...
El seguimiento está en [PR draft #1354](https://github.com/isaacalbala12/Vantare-Simracing-Suite/pull/1354), rama `vantareapp/isa-1347-nightly-parser-perf` desde el SHA integrado. Los intentos `1be6fdf9` y `f7fac47f` pasaron promoción/ratchet, pero el gate frontend midió compacto en 1,656 ms y 1,624 ms respectivamente; el límite se conserva en 1,5 ms. El commit `bae7c02e` reutiliza el buffer UTF-8 y valida que se lea el texto completo, con borde exacto de 72 KiB. `264ceb86` añade una ruta directa para filas compactas comunes de Standings, sin saltar la validación estricta. Suite focal final 41/41, legacy 0,748 ms y compacto 0,814 ms; typecheck, lint y build PASS. La suite completa del intento anterior pasó 481 archivos, 4.036 pruebas (2 omitidas); CI del head actualizado pendiente.
