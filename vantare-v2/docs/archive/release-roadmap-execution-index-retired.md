...
Mapa historico y contractual para llevar Vantare Simracing Suite desde la beta
publica `v0.1.x` hasta el release oficial. Linear posee el estado operativo y
cada issue enlaza el plan ejecutable aplicable.
...
Si hay conflicto, aplica `docs/vantare-program/source-ownership.md`: Linear
posee los campos operativos esperados; Git/GitHub y el runtime demuestran lo
observado; ADR y contratos poseen decisiones estables. Este indice y los
roadmaps externos son contexto historico.
...
1. Release 01 - Beta baseline, recomendados y presets (plan histórico no versionado)
2. [Release 02 - Stripe, Supabase, auth y licencias](../superpowers/plans/2026-06-26-release-02-stripe-supabase-licensing.md)
3. [Release 03 - Autoupdater y distribucion](../superpowers/plans/2026-06-27-release-03-autoupdater-distribution-technical-plan.md)
4. Release 04 - Preview avanzada y LayoutStudio profesional (plan histórico no versionado)
5. Release 05 - Data reliability matrix (plan histórico no versionado)
6. Release 06 - iRacing y Assetto Corsa como simuladores (plan histórico no versionado)
7. Release 07 - Assetto Corsa Lua/CSP Overlay Pack (plan histórico no versionado)
8. Release 08 - Layouts por sesion y visibilidad condicional (plan histórico no versionado)
9. Release 09 - Widget pack y data blocks (plan histórico no versionado)
10. Release 10 - Multiclase, headers y standings release (plan histórico no versionado)
11. Release 11 - Engineer radio overlay y calendario Discord (plan histórico no versionado)
12. Release 12 - OBS y streaming (plan histórico no versionado)
13. Release 13 - UX, onboarding, i18n y design system (plan histórico no versionado)
14. Release 14 - Docs, soporte, legal y release candidate (plan histórico no versionado)
15. Release 15 - Auditoria de seguridad, code review y refactors profundos (plan histórico no versionado)
...
- `HUB-05-B`: implementado, pendiente review/commit — paginas internas v5.2: `TelemetryPage` placeholder honesto, `V52OverlaysHome` con 4 cards reales (perfiles propios cuenta real), `EngineerPage` reorganizado (eventos Wails intactos, sin fake data del HTML), `SettingsPage` con wrapper v5.2 minimo (sin tabs profundas de SETTINGS-01), `LauncherPage` con helper text honesto. Limpieza P3 barata de HUB-05 incluida (casts `as Section`, `Topbar` tipado, reexport muerto de `EMPTY_CALENDAR`, test complaciente de `DashboardPage`).
