...
## D-R4-3 · Ventanas pequeñas escalan; las ultrapanorámicas topan el contenido
**Decisión.** La shell está calibrada a 1920×1080 y declaraba un suelo duro de 1180 px de ancho, así que al reducir la app de escritorio a ~870×780 los paneles se recortaban por la derecha. A partir de ahora, **primero se pliega y después se escala**:

1. **Plegado (sin cambios).** Las reglas que ya existían siguen mandando y leen el viewport **real**: columna auto-plegada ≤ 1152 px, Inicio compacto ≤ 940 px de alto, ajuste ≤ 790 px de alto. Chromium no ajusta las media queries al `zoom`, así que CSS y JS (`AUTO_COLLAPSE_WIDTH`) ven exactamente lo mismo y no pueden contradecirse.
2. **Escalado (nuevo).** Solo por debajo de los suelos que quedan **después** de plegar: 1180 px de ancho y 790 px de alto. Factor `min(w/1180, h/790, 1)` con suelo `0.6`, en `src/hub/orbit/orbit-zoom.ts`, aplicado por `useOrbitResponsiveZoom` con `requestAnimationFrame` en cada `resize`. Nunca amplía: a 1920×1080 y por encima el factor es exactamente 1 y no se deja `zoom` escrito. A 870×780 el factor es 0.737 y la shell se maqueta a 1180×1058 para acabar midiendo la ventana exacta.
3. **Por debajo del suelo** (ventana absurdamente pequeña, < 708×474) la shell deja de encoger y **desplaza dentro de su contenedor** en lugar de recortar; la página sigue sin sacar barras.

**El `zoom` va en `<html>`, no en `.orbit-root`.** El `Select`, el `Drawer` y los toasts se portalan a `document.body`, fuera de la shell: desde la raíz escalan igual que todo lo demás. A cambio conviven dos espacios de coordenadas —los rects y `window.innerWidth/Height` en px reales, la maquetación del documento en px de maquetación— y las capas flotantes que se posicionan a partir de un rect medido tienen que convertir. `ui/orbit/layout-scale.ts` hace esa conversión y `Select` la usa; sin ella la lista aparecía 186 px desplazada del disparador. Por el mismo motivo `useHubResponsiveZoom` (el zoom de V52, que amplía por encima de 1080 px de alto) se apaga con Orbit activo: los dos escribían `documentElement.style.zoom`.

**Ultrapanorámicas.** El contenido de las vistas ya iba topado y centrado a `--orbit-content-max` (1508 px, § 3.5), así que a 3440 o 5120 px nunca hubo rejillas de 3000 px: hay márgenes. Lo que se añade es un escalón para no desperdiciar el ancho: **a partir de 2200 px reales el tope sube a 1760 px**, el mayor valor que mantiene la longitud de línea legible y las proporciones calibradas de las rejillas. El fondo del workspace sigue ocupando la ventana entera y las vistas con `height: 100%` siguen llenando el alto. **El Studio queda exento del tope**: su lienzo 16:9 sí gana con todo el ancho disponible.

**Verificación.** `visual:orbit-responsive` (`scripts/orbit-responsive-visual.mjs`) recorre Inicio y Ajustes a 870×780, 1024×640, 1280×720, 1366×768, 1600×900, 1920×1080, 2560×1080, 3440×1440 y 5120×1440, más el caso de suelo a 640×420 y las capas flotantes a 870×780. Afirma el factor esperado en cada tamaño, que la shell mide la ventana exacta, que no hay scroll de página, que ningún panel se sale sin un contenedor que lo desplace, el plegado de la columna donde toca, y el tope y el centrado del contenido en ultrapanorámicas. Capturas en `evidence/porte/01-shell/responsive/`. El Studio no entra en este harness —su propio `visual:orbit-studio` lo cubre— porque su harness no llega a montar el editor dentro de este script y se queda en «Cargando perfiles…».

| Resolución | Relación | Factor | Columna | Contenido |
|---|---|---|---|---|
| 870×780 | ~10:9 | 0.74 | plegada | tope 1508, ocupa todo el ancho útil |
| 1024×640 | 16:10 | 0.81 | plegada | tope 1508, ocupa todo el ancho útil |
| 1280×720 | 16:9 | 0.91 | desplegada | tope 1508, ocupa todo el ancho útil |
| 1366×768 | 16:9 | 0.97 | desplegada | tope 1508, ocupa todo el ancho útil |
| 1600×900 | 16:9 | 1 | desplegada | tope 1508, ocupa todo el ancho útil |
| 1920×1080 | 16:9 | 1 | desplegada | tope 1508 centrado |
| 2560×1080 | 21:9 | 1 | desplegada | tope 1760 centrado |
...
## D-R4-5 · El escalado sigue al gesto: una escritura por frame, medido en la app real

**Contexto.** Con D-R4-4 aplicado, Isaac sigue reportando lo mismo en la app de escritorio real (Wails/WebView2, Windows 11): al reducir la ventana con un gesto rápido «la UI Orbit sigue reduciendo su tamaño muy lento… no es fluido… parece algo intrínseco de la UI». D-R4-4 se calibró contra un harness headless que redimensiona el viewport de Playwright; eso **no reproduce el gesto real**, que entra en el bucle modal de Windows (`WM_ENTERSIZEMOVE`). Había que medir dentro de la app.

**Cómo se midió.** Ver `evidence/porte/01-shell/responsive/real-resize-profile.md` para el protocolo completo y las tablas. En resumen: gancho `VANTARE_WEBVIEW_DEBUG_PORT` en `cmd/vantare/main.go` que abre el protocolo DevTools del WebView2 (la variable de entorno estándar no vale: Wails la sobrescribe al crear el entorno); arrastre real de la esquina con `WM_SYSCOMMAND SC_SIZE` + `SetCursorPos` en 40 pasos de 1900×1020 a 900×700 en 700 ms; captura por CDP de la cadencia de `rAF`, las long tasks y los deltas de `Layout/RecalcStyle/Script/TaskDuration`; y una métrica nueva de **desajuste visual** (distancia por frame entre la escala aplicada y la que pide el viewport). Cinco condiciones, tres corridas cada una. Interruptor de diagnóstico `vantare.v03orbit.zoomOff` para medir con y sin escalado sin recompilar.

**Diagnóstico medido.** El hilo principal **no es el cuello**: durante el arrastre modal real el renderer va a 60 Hz clavados (p50 16.7 / p95 16.8 ms) en las cinco condiciones, con **cero long tasks**, y layout + estilo suman ~250 ms repartidos en 4.6 s (~5 % del presupuesto). La shell legada V52 se comporta igual, así que tampoco es la plataforma. Lo que falla es el propio throttling de D-R4-4: para los **38-40** eventos de `resize` del gesto la shell aplicaba solo **4 escalones** de escala, dejando el contenido descuadrado respecto al marco **19 frames (~320 ms)** y cuadrándolo **50-82 ms después de soltar**. Eso es literalmente lo que se percibe como «sigue reduciéndose lento»: el marco salta al instante y el contenido lo persigue a saltos gruesos, tarde.

La premisa de D-R4-4 —que la escritura de `zoom` costaba 6.0 ms de mediana— no se sostiene en la app real: la condición **con** zoom sale más barata que la condición **sin** zoom (layout 117 vs 133 ms, estilo 137 vs 174 ms), porque escalado el contenido se renderiza más pequeño. Los 6 ms se medían con lectura forzada de layout inmediatamente después de escribir, que es justo lo que el navegador no tiene que hacer si nadie lee.

**Decisión.**

1. **El factor se escribe una vez por frame**, coalescido con `requestAnimationFrame`. Fuera el throttling de 120 ms y el reposo de 180 ms. Varios `resize` dentro del mismo frame producen una sola escritura, y ningún frame se queda sin la escala que le toca. La bandera de coalescencia se marca *antes* de encolar, para que un `rAF` síncrono no deje el gesto colgado.
2. **Se conserva `data-orbit-resizing`** y la congelación de transiciones y `backdrop-filter` acotada a `.orbit-root`: sigue haciendo falta para que las transiciones no se re-disparen en cada escalón. Su reposo baja a 120 ms y **ya no retrasa el escalado**, solo levanta la marca.
3. **No se añade containment, ni se quitan `backdrop-filter`, ni se pausan los `ResizeObserver`, ni se mete un store de viewport, ni se aplica el zoom solo al soltar.** Ninguna de esas medidas está justificada por la medición y todas tienen coste visual o estructural: el layout consume ~4 % del presupuesto y `ScriptDuration` es 20-56 ms en 4.6 s. «Aplicar el zoom solo al soltar» habría sido la dirección contraria a lo medido: es la versión extrema del defecto que causaba el síntoma.
4. **Se mantiene el escalado** (no se sustituye por plegar y aceptar scroll interno): escalar no es incompatible con la fluidez en WebView2, el escalado por frame va a 60 Hz.
...
## D-CAL-24 · Lectura visual del Calendario para lanzamiento

**Decisión de Isaac, 24/09/2026.** En la build Wails, el detalle de la derecha solo se muestra tras elegir una serie. Día agrupa las salidas reales en franjas de 15 minutos, sin inventar ni suprimir las cadencias publicadas. Cada celda de Mes abre Día. `eventKind` distingue diaria, semanal y especial con colores sobrios diferentes. Timeline se limita a las salidas de la próxima hora con marcas de inicio breves y zoom que cambia el ancho útil. El estado activo del rail y de las filas deja de dibujar la barra vertical carmín. El Calendario adapta controles, paneles y rejillas al zoom y al ancho de la ventana. Este ajuste sustituye las medidas de 24 horas y detalle permanente del briefing 06 inicial.

**Revisión de Isaac, 25/09/2026.** Se elimina el sombreado rojo de la franja actual en Día. Semana se reserva para las series semanales y especiales. Timeline aumenta el margen de los puntos y rótulos, mantiene los rótulos visibles durante el scroll, retira la línea vertical roja que cruzaba los rótulos, expone desplazamiento horizontal por botones y amplía el eje hasta 4× con indicador visible.
