...
- El contrato de sesión exige `Event` y `Session` completos desde el primer lote. Dentro del epoch ambos permanecen estables según `SameSession`; un header parcial no desactiva el control. Un reset exacto de epoch en sequence 1 puede cambiar ambos. `Vehicle`, `Team` y `Driver` pueden cambiar dentro de la sesión.
...
- Decisiones y guía de verificación: `docs/telemetry-core/runtime-reducer.md`. Evidencia fresca tras corrección: core x20, Telemetry Core completo, guard ADR 0004, vet focal, suite global Go y race focal PASS. Race usó el GCC UCRT64 ya instalado con configuración solo de proceso; no se instaló ni cambió el sistema. Fuzz con oracle 10 s PASS con 2.508.370 ejecuciones. Benchmark Windows/amd64 con 64 vehículos, cinco repeticiones: 8,77–9,61 µs/op, 36.264 B/op y 5 allocs/op. El vet amplio conserva seis warnings `unsafe.Pointer` heredados fuera del diff.
