...
- cuerpo: Paridad CrewChief por cortes T0–T8. T0a y T0b tienen referencias y casos independientes revisados; T1 calcula relaciones sintéticas de clase y pista, y T2 añade muestreo por puntos o sectores con historial separado por rival. Ambas rutas están desactivadas por defecto y aún sin conexión a la voz. Faltan verificar las señales LMU reales y completar clasificación, silencios, cadencia, consultas y escucha. Los hechos seguirán siendo deterministas; el LLM cloud redactará con StyleGate y el fallback offline usará frases locales precacheadas. Este avance no acredita paridad funcional completa.
- cuerpo.en: CrewChief parity in T0–T8 stages. T0a and T0b have reviewed references and independent cases; T1 computes synthetic class and track relations, and T2 adds point or sector sampling with separate history per rival. Both paths are disabled by default and not yet connected to voice. Real LMU signals, classification, silence rules, cadence, queries and listening remain to be verified. Facts remain deterministic; the cloud LLM will write behind a StyleGate and offline fallback will use local precached phrases. This progress does not establish full functional parity.
- cuerpo.pt: Paridade CrewChief em etapas T0–T8. T0a e T0b têm referências e casos independentes revistos; T1 calcula relações sintéticas de classe e pista, e T2 acrescenta amostragem por pontos ou setores com histórico separado por rival. Ambas as rotas estão desativadas por padrão e ainda sem ligação à voz. Falta verificar sinais LMU reais e concluir classificação, silêncios, cadência, consultas e escuta. Os factos continuam determinísticos; o LLM cloud redigirá com StyleGate e o fallback offline usará frases locais pré-armazenadas. Este avanço não comprova paridade funcional completa.
- cuerpo.it: Parità CrewChief in fasi T0–T8. T0a e T0b dispongono di riferimenti e casi indipendenti revisionati; T1 calcola relazioni sintetiche di classe e pista, e T2 aggiunge campionamento per punti o settori con uno storico separato per avversario. Entrambi i percorsi sono disattivati per impostazione predefinita e non ancora collegati alla voce. Restano da verificare i segnali LMU reali e completare classificazione, silenzi, cadenza, query e ascolto. I fatti restano deterministici; il LLM cloud scriverà con StyleGate e il fallback offline userà frasi locali in cache. Questo avanzamento non dimostra la piena parità funzionale.
- etiqueta: Plan
- etiqueta.pt: Plano
- etiqueta.it: Piano
