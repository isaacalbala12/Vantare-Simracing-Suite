...
function activate(): void {
  ...
  const activeProfileId = settingsStore.getActiveOverlayProfileId();
  if (activeProfileId !== snapshot.activeProfileId) {
    snapshot = { ...snapshot, activeProfileId };
  }
  ...
  const unsubSettings = settingsStore.subscribeActiveOverlayProfileId(() => {
    ...
    if (next === snapshot.activeProfileId) return;
