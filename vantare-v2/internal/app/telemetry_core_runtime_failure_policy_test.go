...
func TestFailureClassificationTable(t *testing.T) {
	tests := []struct {
		...
		{name: "transport delta mismatch", err: telemetrytransport.ErrDeltaMismatch, want: failureProductOrPayload},
		...
		{name: "transport fact sequence", err: telemetrytransport.ErrFactSequence, want: failureProductOrPayload},
