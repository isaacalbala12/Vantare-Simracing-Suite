...
- Compatibilidad/lifecycle fail-closed: buffer corto es `ErrIncompatibleBuffer` terminal; mapping ausente o lectura perdida devuelve `ErrDisconnected` retryable. `known` exige evidencia estructural positiva runtime sin publicar identidad; all-zero/plausible sin evidencia queda `unknown`, fingerprint `insufficient` y campos por offsets `missing`. Reloj inmóvil pasa a `stale`; incompatibilidad domina stale; retrocesos distinguen reset y wrap.
...
- Teardown: `driver.ErrTeardown` y resultado ligado a generación permiten que `DriverManager.Stop` propague fallos de close a uno o varios callers sin heredarlos tras restart. El seam Win32 valida el return value antes de last-error y cubre map failure, close success/error, doble/concurrent close y Snapshot post-close.
- Evidencia review fresca: focal core/driver/LMU repetido 20 veces PASS; fuzz explícito 10 s PASS (41.091 ejecuciones); benchmark estabilidad+parse 21,50–22,48 µs/op, 200 B/op y 4 allocs/op (margen >740x frente a 16,67 ms a 60 Hz); `go test ./internal/telemetry/... -count=1` y guard ADR PASS; `git diff --check` PASS. Vet core/driver PASS y LMU conserva solo el warning localizado de `unsafe.Pointer` del mmap; `-race` se intentó y no arranca con `CGO_ENABLED=0`.
