...
if ($UpdateTrustSource) {
    ...
    $expected = (Get-Content -Raw -LiteralPath $trustSource).Trim()
    Write-Host "TRUST_MISMATCH expected=$expected"
    Write-Host "TRUST_MISMATCH generated=$generated"
    Write-Host "TRUST_MISMATCH manifestHash=$manifestHash helperHash=$helperHash dllSha256=$(Get-Sha256 (Join-Path $output "duckdb.dll"))"
    Write-Host "TRUST_MISMATCH files=$($manifestFiles | ConvertTo-Json -Compress)"
    throw "Runtime manifest is not trusted by the app. Review it, then rerun with -UpdateTrustSource. expected=$expected generated=$generated"
}
