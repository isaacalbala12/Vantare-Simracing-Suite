...
Seguimiento correctivo en la rama `vantareapp/isa-1347-nightly-parser-perf`, basada en ese SHA de Nightly. El primer head `17c0e91a` optimizó el tamaño UTF-8 con un patrón ASCII; tras cumplir promoción path y ratchet, el gate frontend volvió a fallar en compacto a 1,656 ms frente a 1,5 ms (head `1be6fdf9`). El commit `bae7c02e` reemplaza el escaneo por codificación sobre un buffer UTF-8 reutilizado, conserva la comprobación exacta de Unicode y el límite de 72 KiB; la regresión cubre el tamaño exacto ASCII y Unicode, y ambos excesos. PR draft [#1354](https://github.com/isaacalbala12/Vantare-Simracing-Suite/pull/1354), CI del head actualizado pendiente.

Evidencia local: pruebas dirigidas de contrato/rendimiento sobre el código final 24/24; muestra legacy 0,684 ms y compacto 0,844 ms; suite completa sobre el primer intento 481 archivos, 4.036 PASS y 2 omitidas; typecheck, lint y build pasan sobre el código final. El gate remoto de #1354 debe validar el head exacto. Sin cambios de roadmap, promoción, release ni afirmación de prueba física; LMU activo en Windows/Desktop/OBS sigue pendiente.
