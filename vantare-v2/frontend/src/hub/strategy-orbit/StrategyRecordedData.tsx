...
function compactSessionLabel(value: string): string {
  ...
  const dated = /^(\d{4}-\d{2}-\d{2})_(.+)$/.exec(withoutExtension);
  return dated ? `${dated[1]} · ${dated[2].replaceAll("_", " ")}` : withoutExtension.replaceAll("_", " ");
}
...
export function StrategyRecordedData({ controller, sessions, sessionLabels = {}, selectedRevisions = [], catalog = [], busy, onSources, onPendingChange, view, onViewChange, t }: {
  ...
  return <section className="strategy-recorded-data" aria-labelledby="recorded-data-title" data-ready={Boolean(editor)}>
    ...
    <div className="strategy-recorded-data__grid">
      <section className="strategy-recorded-data__observations">
        <div className="strategy-recorded-data__source"><label>{t("strategy.data.source")}<select value={editor?.session.opened.sessionId ?? ""} disabled={locked || formDirty || controller.unresolved} onChange={event => { const session = sessions.find(item => item.opened.sessionId === event.target.value); if (session) { clearForm(); setColumn(""); void controller.load(session); } }}>
          <option value="">{t("strategy.journey.choose")}</option>{sessions.map(session => {
            const label = sessionLabels[session.candidateId] || [session.combination?.trackName, session.combination?.carName].filter(Boolean).join(" · ") || t("strategy.recorded.unnamed");
            return <option key={session.opened.sessionId} value={session.opened.sessionId} aria-label={label} title={label}>{compactSessionLabel(label)}</option>;
          })}
        </select></label>{editor ? <Button variant="ghost" disabled={locked || formDirty || controller.unresolved} onClick={onSources}>{t("strategy.data.sources")}</Button> : null}</div>
        <div className="strategy-recorded-laps__views"><h3>{t("strategy.data.observations")}</h3><label className="strategy-recorded-laps__view-select"><span>{t("strategy.data.observations")}</span><select value={view} disabled={locked || formDirty || Boolean(editor?.request)} onChange={event => { clearForm(); onViewChange(event.target.value as RecordedDataView); }}><option value="classification">{t("strategy.classification.tab")}</option><option value="stints">{t("strategy.stints.tab")}</option><option value="laps">{t("strategy.laps.back")}</option><option value="samples">{t("strategy.laps.advanced")}</option></select></label><span>{view !== "classification" ? <Button size="sm" variant="ghost" disabled={locked || formDirty || Boolean(editor?.request)} onClick={() => { clearForm(); onViewChange("classification"); }}>{t("strategy.classification.tab")}</Button> : null}{view !== "stints" ? <Button size="sm" variant="ghost" disabled={locked || formDirty || Boolean(editor?.request)} onClick={() => { clearForm(); onViewChange("stints"); }}>{t("strategy.stints.tab")}</Button> : null}{view === "laps" ? <Button size="sm" variant="ghost" disabled={locked || formDirty || Boolean(editor?.request)} onClick={() => { clearForm(); onViewChange("samples"); }}>{t("strategy.laps.advanced")}</Button> : <Button size="sm" variant="ghost" disabled={locked || formDirty || Boolean(editor?.request)} onClick={() => { clearForm(); onViewChange("laps"); }}>{t("strategy.laps.back")}</Button>}{view === "classification" ? <Button size="sm" variant="ghost" disabled={locked || formDirty || Boolean(editor?.request)} onClick={() => { clearForm(); onViewChange("samples"); }}>{t("strategy.laps.advanced")}</Button> : null}</span></div>
        {view === "samples" ? <p className="strategy-recorded-data__muted">{t("strategy.data.advancedHint")}</p> : null}
        {!editor ? <div className="strategy-recorded-data__empty"><Icon name="i-telemetria" size={44} /><strong>{t("strategy.data.chooseSource")}</strong><p>{t("strategy.data.chooseSourceHint")}</p><Button variant="primary" disabled={locked || controller.unresolved} onClick={onSources}>{t("strategy.data.sources")}</Button></div> : view === "laps" ? <RecordedLapList page={editor.lapPage} family={family} proposals={editor.familyUses ?? []} selected={familyForm?.row} locked={locked || formDirty || Boolean(editor.request)} t={t}
          ...
          <div className="strategy-recorded-data__channels"><label>{t("strategy.data.channel")}<select value={page?.channel_id ?? ""} disabled={locked || formDirty} onChange={event => { clearForm(); setColumn(""); void controller.page(event.target.value, 0); }}><option value="" disabled>{t("strategy.journey.choose")}</option>{editor.session.opened.session.channels.map(item => <option key={item.id} value={item.id}>{item.source_name}{item.unit.symbol ? ` · ${item.unit.symbol}` : ""}</option>)}</select></label>
            {channel ? <label>{t("strategy.data.column")}<select value={currentColumn} disabled={locked || formDirty} onChange={event => { clearForm(); setColumn(event.target.value); }}>{channel.columns.map(item => <option key={item.name} value={item.name}>{item.name === "value" ? t("strategy.data.column") : item.name}</option>)}</select></label> : null}</div>
    ...
    <footer className="strategy-recorded-data__footer"><span><Icon name="i-lock" size={19} />{t("strategy.recorded.originals")}</span>{controller.busy && !controller.projecting ? <span role="status">{t("strategy.data.working")} <Button variant="ghost" onClick={controller.cancel}>{t("strategy.recorded.cancel")}</Button></span> : null}</footer>
  </section>;
}
