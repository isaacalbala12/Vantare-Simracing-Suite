...
- Implementada la matriz versionada `v1` de autoridad LMU para los 14 campos actualmente demostrados: cinco solapados con Shared Memory preferida y REST alternativa semánticamente equivalente, seis exclusivos Shared Memory y tres exclusivos REST.
- La fusión single-writer vive dentro de `internal/telemetry/drivers/lmu` y publica lotes canónicos por campo. Decide por presencia, validez, freshness y TTL (Shared Memory 500 ms; REST 2 s); cero/false siguen siendo valores válidos. Nunca promedia conflictos, sustituye bloques ni inventa fallback.
- Cada lote incluye una decisión determinista por campo y como máximo cinco diagnósticos de conflicto con solo ID/fuentes, sin valores, raw, JSON, PII ni rutas. La salida canónica deja vacío el snapshot de adquisición REST para impedir que un consumidor salte la autoridad por campo. No hay proyecciones, Engineer, Overlay, Strategy, Wails/SSE, composition root, dependencias o wiring productivo.
- Tests TDD cubren tabla completa, preferred stale/invalid/missing, REST parcial, recuperación, cero válido, conflictos, TTL, determinismo, fuzz y benchmark; lifecycle/cancelación continúan cubiertos por el driver.
...
- Evidencia fresca: focal LMU repetido 20 veces PASS; `go test ./internal/telemetry/... -count=1`, guard ADR 0004, build frontend y suite global `go test ./... -count=1` PASS. Fuzz de fusión 10 s PASS (~400.181 ejecuciones). Benchmark 669,8–813,6 ns/op, 480 B/op y 2 allocs/op. Review adversarial sin P0/P1/P2 tras corregir la prioridad `stale válido > invalid`; vet focal conserva solo dos warnings `unsafe.Pointer` Win32 heredados y `-race` sigue no disponible con `CGO_ENABLED=0`/gcc ausente.
- Estado: implementación preparada para commit/push, PR draft apilada sobre ISA-33 e `In Review`; sin merge a `develop`.
